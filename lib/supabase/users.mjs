...
import { normalizeCompanyActivities } from "../company-activities.mjs";
import { adminFlagsFromProfile } from "../user-admin-flags.mjs";
import { isEmailBanned } from "../banned-users.mjs";
...
export function isSupabaseSchemaMissingError(error) {
  ...
  if (!msg) return false;
  // Unique/duplicate constraint errors mention table names but are NOT missing schema.
  if (
  ...
  return (
    ...
    ((/web_users|web_sessions|web_user_identities/.test(msg) &&
      (msg.includes("does not exist") || msg.includes("schema cache") || msg.includes("fetch failed")))
    )
  );
}

/** Auth API: soha ne küldjünk nyers Postgres / PostgREST hibát a böngészőbe. */
export function friendlyAuthErrorMessage(error, fallback = "Sikertelen művelet.") {
  const code = String(error?.code ?? "");
  if (code === "ACCOUNT_NOT_FOUND") {
    return "Nincs ilyen fiók. Előbb regisztrálj, utána tudsz belépni.";
  }
  if (code === "ACCOUNT_INACTIVE") {
    return String(
      error?.message || "Ez a fiók inaktív. Regisztrálj újra, vagy kérj aktiválást."
    );
  }
  if (code === "EMAIL_NOT_VERIFIED") {
    return String(error?.message || "Előbb aktiváld az emailed.");
  }
  if (code === "EMAIL_ALREADY_REGISTERED") {
    return String(error?.message || "Regisztráció sikertelen. Ha már van fiókod, jelentkezz be.");
  }
  const msg = String(error?.message ?? error ?? "").trim();
  if (
    code === "23505" ||
    /duplicate key|unique constraint|violates unique/i.test(msg)
  ) {
    return "Nincs ilyen fiók. Előbb regisztrálj, utána tudsz belépni.";
  }
  if (/postgres|postgrest|pgrst|sqlstate|relation "|column "|schema cache/i.test(msg)) {
    return fallback;
  }
  return msg || fallback;
}
...
function radiusProfileKm(profile, existing, key, fallback = 30, max = 100) {
  const raw = profile[key] != null ? profile[key] : existing[key];
  let n = Number(raw);
...
function scrubPrivateStreet(profile) {
  const next = { ...profile };
  if (normalizeAccountType(next.accountType) !== "business") {
    next.street = "";
  }
  return next;
}
...
function profileFromRow(row) {
  ...
  return scrubPrivateStreet(merged);
}
...
async function createSession(userId) {
  ...
  try {
    await sb().from("web_users").update({ last_login_at: new Date().toISOString() }).eq("id", userId);
  } catch {
    /* last_login_at column may be missing until migration */
  }
...
async function findUserForLogin(identifier) {
  ...
  if (normalized.includes("@")) {
    const { data, error } = await sb().from("web_users").select("*").eq("email", normalized).maybeSingle();
    if (error) throw error;
    return data;
  }
  ...
  const { data: byPrefix, error: prefixErr } = await sb()
    .from("web_users")
    .select("*")
    .ilike("email", `${key}@%`)
    .limit(2);
...
function oauthAllowsCreate(identity) {
  const t = String(identity?.accountType ?? "").trim().toLowerCase();
  return t === "private" || t === "business";
}

function throwAccountInactive() {
  const err = new Error("Ez a fiók inaktív. Regisztrálj újra, vagy kérj aktiválást.");
  err.code = "ACCOUNT_INACTIVE";
  throw err;
}

function throwAccountNotFound() {
  const err = new Error("Nincs ilyen fiók. Előbb regisztrálj, utána tudsz belépni.");
...
export async function findOrCreateOAuthUser(identity) {
  ...
  const canCreate = oauthAllowsCreate(identity);
  ...
  if (identityRow?.user_id) {
    ...
    if (!user?.id) {
      // Orphan identity after user delete — drop it and continue (create only via regisztráció).
      await sb().from("web_user_identities").delete().eq("id", identityRow.id);
    } else {
      if (!user.emailVerified) {
        if (!canCreate) throwAccountInactive();
        await sb()
          .from("web_users")
  ...
  if (!userRow) {
    // Belépés oldal: ne hozzon létre fiókot (törlés után se). Új fiók csak regisztrációnál.
    if (!canCreate) throwAccountNotFound();
    ...
    userId = userRow.id;
    const verified = userRow.email_verified === true || Number(userRow.email_verified) === 1;
    if (!verified) {
      if (!canCreate) throwAccountInactive();
      await sb()
        .from("web_users")
...
export async function getUserBySessionToken(token, opts = {}) {
  if (!token) return null;
  try {
    // Ne töröljünk lejárt sessionöket minden API híváskor — ritkán elég.
    ...
    if (opts.light) {
      const { data: row, error: userErr } = await sb()
        .from("web_users")
        .select("id, email, display_name, email_verified, profile_json, created_at, updated_at")
        .eq("id", data.user_id)
        .maybeSingle();
      if (userErr) {
        if (isSupabaseSchemaMissingError(userErr)) return null;
        throw userErr;
      }
      if (!row) return null;
      const email = String(row.email ?? "");
      let adminFlags = adminFlagsFromProfile({});
      try {
        const profile =
          typeof row.profile_json === "string"
            ...
            : row.profile_json && typeof row.profile_json === "object"
              ? row.profile_json
              : {};
        adminFlags = adminFlagsFromProfile(profile);
      } catch {
        /* ignore */
      }
      return {
        ...
        emailVerified: row.email_verified === true || Number(row.email_verified) === 1,
        profile: { ...emptyProfile(), adminFlags },
        adminFlags,
        createdAt: row.created_at,
        updatedAt: row.updated_at,
      };
...
export async function registerUser(email, password, passwordConfirm, accountType) {
  ...
  if (pass.length < 12) throw new Error("A jelszó legalább 12 karakter legyen.");
  if (pass !== confirm) throw new Error("A két jelszó nem egyezik.");

  if (await isEmailBanned(normalized)) {
    const err = new Error("Ez az email cím véglegesen le van tiltva. Új fiók nem hozható létre.");
    err.code = "EMAIL_BANNED";
    throw err;
  }

  const { data: existing } = await sb().from("web_users").select("id, email_verified").eq("email", normalized).maybeSingle();
  if (existing?.email_verified) {
    const err = new Error("Regisztráció sikertelen. Ha már van fiókod, jelentkezz be.");
...
export async function requestPasswordReset(email) {
  ...
  const { data: row } = await sb().from("web_users").select("*").eq("email", normalized).maybeSingle();
  if (!row || !hasPasswordHash(row) || !row.email_verified) {
  ...
  const { error } = await sb()
    .from("web_users")
    .update({
      password_reset_token_hash: tokenHash(token),
...
export async function resetPasswordByToken(rawToken, password, passwordConfirm) {
  const token = String(rawToken ?? "").trim();
  ...
  await sb().from("web_sessions").delete().eq("user_id", row.id);
  return { ok: true };
}
...
export async function mergeUserProfileJson(userId, patch = {}) {
  ...
  let parsed = {};
  try {
    if (typeof row.profile_json === "string") parsed = JSON.parse(row.profile_json || "{}");
    else if (row.profile_json && typeof row.profile_json === "object") parsed = { ...row.profile_json };
  } catch {
    parsed = {};
  }
...
export async function getUserDetailsForAdmin(userId) {
  ...
  const listings = await listListingsForOwner(userId);
  return {
    id: row.id,
    email: row.email,
    ...
    profile: profileJson,
    accountType:
      String(profileJson.accountType || "private").toLowerCase() === "business" ||
      String(profileJson.accountType || "").toLowerCase() === "dealer"
        ? "business"
        : "private",
    companyActivities: normalizeCompanyActivities(profileJson.companyActivities),
    adminFlags: adminFlagsFromProfile(profileJson),
    createdAt: row.created_at,
    updatedAt: row.updated_at,
...
function companyFieldsFrom(profile, existing = {}) {
  ...
  return {
    ...
    companyActivities: normalizeCompanyActivities(profile.companyActivities, existing),
  };
}
...
export async function saveUserProfile(userId, profile) {
  ...
  const next = {
    ...
    street:
      accountType === "business" ? strProfileField(profile, existing, "street") : "",
    ...
    accountType,
    searchRadiusKm: radiusProfileKm(profile, existing, "searchRadiusKm", 30, 100),
    recommendationsRadiusKm: radiusProfileKm(profile, existing, "recommendationsRadiusKm", 30, 30),
  };
  ...
  if (existing.adminFlags && typeof existing.adminFlags === "object") {
    next.adminFlags = existing.adminFlags;
  }
...
export async function adminCreateWebUser({
  email,
  displayName,
  password,
  accountType = "private",
  ...
  if (!normalized) throw new Error("Email / felhasználónév kötelező.");
  if (!normalized.includes("@")) {
    const local = normalized.replace(/[^a-z0-9._+-]/gi, "").slice(0, 48);
    if (!local) throw new Error("Érvénytelen felhasználónév.");
    normalized = `${local}@test.bymy.hu`;
  }
  ...
  if (name.length > 40) throw new Error("A megjelenített név maximum 40 karakter lehet.");

  const { data: existing } = await sb().from("web_users").select("id").eq("email", normalized).maybeSingle();
  if (existing) throw new Error("Ez az email / felhasználónév már foglalt.");
  ...
  return getUserDetailsForAdmin(inserted.id);
}

export async function deleteUserAccount(userId) {
  const uid = Number(userId);
  if (!Number.isFinite(uid) || uid <= 0) return { deletedListings: 0 };

  // Ownership is EAV (listing_cells.owner_user_id) — no FK, must delete explicitly.
  ...
  // Fallback: trim / numeric coercion if exact string match found nothing.
  if (!listingIds.size) {
  ...
  for (let i = 0; i < ids.length; i += 100) {
    ...
    const { error: wipeErr } = await sb().from("listing_cells").delete().in("listing_id", chunk);
    if (wipeErr) throw wipeErr;
    const { data: removed, error: listErr } = await sb()
      .from("listings")
      .delete()
      .in("id", chunk)
      .select("id");
  ...
  await sb().from("web_sessions").delete().eq("user_id", uid);
  await sb().from("web_user_identities").delete().eq("user_id", uid);
  const { error: userErr } = await sb().from("web_users").delete().eq("id", uid);
...
export async function listWebUsersForAdmin() {
  const { data, error } = await sb()
    ...
    .order("id");
  if (error) {
    // Régi séma: last_login_at / profile_json nélkül
    const retry = await sb()
      .from("web_users")
      .select("id, email, display_name, email_verified, created_at, updated_at")
      .order("id");
    if (retry.error) throw retry.error;
    const counts = await listingCountsByOwner();
      ...
      const profile =
        typeof row.profile_json === "string"
          ...
          : row.profile_json && typeof row.profile_json === "object"
            ? row.profile_json
            : {};
      const rawType = String(profile.accountType || "private").toLowerCase();
      accountType = rawType === "business" || rawType === "dealer" ? "business" : "private";
      companyActivities = normalizeCompanyActivities(profile.companyActivities);
      adminFlags = adminFlagsFromProfile(profile);
    } catch {
      /* ignore */
    }
...
async function listingCountsByOwner() {
  const map = new Map();
  try {
    const { data, error } = await sb()
      .from("listing_cells")
    ...
    for (const row of data || []) {
      ...
      if (!byOwner.has(id)) byOwner.set(id, new Set());
      byOwner.get(id).add(row.listing_id);
    }
    for (const [id, set] of byOwner) map.set(id, set.size);
  } catch {
    /* ignore */
  }
  return map;
...
async function listListingsForOwner(userId) {
  try {
    const { data: cells, error } = await sb()
      .from("listing_cells")
      .select("listing_id")
      .eq("field_key", "owner_user_id")
      .eq("value", String(userId));
    if (error) throw error;
    const ids = [...new Set((cells || []).map((c) => c.listing_id))].slice(0, 100);
    if (!ids.length) return [];
    const { data: listings, error: lErr } = await sb()
      .from("listings")
      .select("id, hirdetes_cime, status, created_at, updated_at")
      .in("id", ids)
      .order("updated_at", { ascending: false });
    if (lErr) throw lErr;
