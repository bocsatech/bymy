...
/** Rövid márkanevek, amik nagybetűsek maradnak (BMW, VW). */
const ACRONYM_BRANDS = new Set(["BMW", "VW", "MG", "DS", "BYD", "GMC", "RAM"]);

/** ALFA ROMEO → Alfa Romeo (cím, breadcrumb) */
...
function formatTitleBrandCase(title) {
  ...
  if (!text) return "";
  // Ha a cím csupa nagybetűs (vagy a márka az), szépítsük
  return text
    .split(/(\s+|\(|\))/)
    .map((part) => {
...
/** Gyártmány + Típus (a modell mező) + év. A rejtett tipus-szöveg nem a kártyacím. */
export function composeVehicleTitle(data = {}) {
  ...
  // Katalógus-kulcs (LAND → LAND ROVER); megjelenítés title-case
  const gyartmany = formatBrandDisplayName(gyartmanyKey || gyartmanyRaw);
  // Típuskód (NX, X5, A4) maradjon eredeti írásmódban
