...
test("buildSellerInventoryContact: értékesítők + telefon + cím", () => {
  ...
  assert.equal(pub.sellerAvatarUrl, "data:image/png;base64,xx");
  assert.equal(pub.staff[0].photoUrl, "data:image/png;base64,xx");
});

test("publicSellerInventoryContact: túl nagy data: képet nem ad ki", () => {
  const huge = `data:image/png;base64,${"A".repeat(260_000)}`;
  const pub = publicSellerInventoryContact({
    sellerName: "X",
    sellerAvatarUrl: huge,
    staff: [{ name: "A", photoUrl: huge }],
    phonesMasked: [],
    hasPhone: false,
    addressLines: [],
    mapQuery: "",
  });
  assert.equal(pub.sellerAvatarUrl, "");
  assert.equal(pub.staff[0].photoUrl, "");
});
