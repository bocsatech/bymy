...
test("cdn extract: listingId + HQ from thumb paths", () => {
  ...
  assert.match(cars[0].visibleTitle, /Mercedes-Benz E 250/i);
  assert.equal(cars[0].modell, "E");
  assert.match(cars[0].tipus, /250 CDI/i);
...
test("dealer import script: extract + Bearer save loop", async () => {
  ...
  try {
    ...
    assert.match(posts[0].pages[0].visibleTitle, /Mercedes-Benz E 250/i);
    assert.equal(posts[0].pages[0].modell, "E");
