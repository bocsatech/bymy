/**
 * Bocsatech admin: hirdetés-pillanatképek (JSON.gz) + részleges visszaállítás.
 * A teljes S2 pg_dump / storage tar továbbra is a katasztrófa-mentés.
 */
...
function filterListings(listings, { category = "all", userId = null, listingId = null } = {}) {
...
export async function previewAdminBackup(id, filters = {}) {
  ...
  const matched = filterListings(data.listings || [], filters);
  return {
    id: data.id,
...
async function restoreOneListing(row) {
...
export async function restoreAdminBackup(id, filters = {}) {
  const data = await readGzipJson(snapshotPath(id));
  let matched = filterListings(data.listings || [], filters);
  if (Array.isArray(filters.listingIds) && filters.listingIds.length) {
    const want = new Set(filters.listingIds.map((x) => Number(x)).filter((n) => Number.isFinite(n) && n > 0));
    matched = matched.filter((row) => want.has(Number(row.id)));
  }
  if (!matched.length) {
  ...
  for (const row of matched) {
    restored.push(await restoreOneListing(row));
  }

  // Bump identity if postgres sequence lags (best-effort via SQL not exposed — skip)
  return {
    ok: true,
