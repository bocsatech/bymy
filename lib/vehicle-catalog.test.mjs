...
test("buildVehicleCatalog: típushoz ajtó és üzemanyag is menthető", () => {
  const rows = parseCsvText(`Gyartmany,Modell,Tipus,Ajtok,Uzemanyag,EvTol,EvIg
ABARTH,500,500 1.4,3 ajtós,Benzin,2007,2015`);
  const entry = buildVehicleCatalog(rows, "details.csv").tipusok["ABARTH|500"][0];
  assert.deepEqual(entry.ajtok, ["3 ajtós"]);
  assert.deepEqual(entry.uzemanyag, ["Benzin"]);
  assert.equal(entry.evTol, 2007);
  assert.equal(entry.evIg, 2015);
});
...
test("findCatalogModel: a Típus a katalógus modell, nem a JS-figyelmeztetés", () => {
  const catalog = sampleCatalog();
  assert.equal(findCatalogModel("ABARTH", "500 Coupe 1.4 TJet 140", catalog), "500");
  assert.equal(findCatalogModel("ABARTH", "böngésző nem támogatja a JavaScript-et.", catalog), "");
  assert.equal(normalizeBrand("LAND"), "LAND ROVER");
});

test("findCatalogBrand: több szavas márka a cím elején", () => {
  const catalog = {
    gyartmanyok: ["ALFA ROMEO", "AUDI", "LAND ROVER", "VOLVO"],
    modellek: {
      "ALFA ROMEO": ["GIULIA"],
      AUDI: ["A4"],
  ...
  assert.equal(findCatalogBrand("ALFA ROMEO GIULIA 2.0", catalog), "ALFA ROMEO");
  assert.equal(findCatalogBrand("LAND ROVER DISCOVERY 5", catalog), "LAND ROVER");
  assert.equal(findCatalogBrand("VOLVO V90 2.0", catalog), "VOLVO");
});

test("stripLeadingCatalogToken: márka/modell levágása", () => {
  assert.equal(stripLeadingCatalogToken("VOLVO V90 2.0 T6", "VOLVO"), "V90 2.0 T6");
  assert.equal(stripLeadingCatalogToken("V90 2.0 T6", "V90"), "2.0 T6");
  assert.equal(stripLeadingCatalogToken("TRANSPORTER T4 1.9", "TRANSPORTER"), "T4 1.9");
});
