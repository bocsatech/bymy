...
async function countHitsSinceSupabase(sinceIso) {
  const sb = getSupabase();
  const [{ count: hits, error }, { count: unique, error: uErr }] = await Promise.all([
    sb.from("site_page_hits").select("*", { count: "exact", head: true }).gte("created_at", sinceIso),
    sb
      .from("site_visitor_sessions")
  ...
  if (error) throw error;
  if (uErr) throw uErr;
  return { hits: hits ?? 0, unique: unique ?? 0 };
}
...
export async function getVisitorPageHits(visitorId, { limit = 200 } = {}) {
...
export async function getVisitorAdminStats() {
  ...
  if (isSupabaseBackend()) {
    try {
      ...
      const dayIso = new Date(new Date().setHours(0, 0, 0, 0)).toISOString();
      const weekIso = new Date(Date.now() - 7 * 864e5).toISOString();
      const monthIso = new Date(Date.now() - 30 * 864e5).toISOString();
      const [onlineRes, daily, weekly, monthly, devicesRes] = await Promise.all([
        sb
          .from("site_visitor_sessions")
          .select("*", { count: "exact", head: true })
          .gte("last_seen_at", onlineIso),
        countHitsSinceSupabase(dayIso),
        countHitsSinceSupabase(weekIso),
        countHitsSinceSupabase(monthIso),
        sb
          .from("site_visitor_sessions")
          .select(
            "id, ip, device_name, device_type, device_label, browser, os, user_agent, language, screen, timezone, viewport, pixel_ratio, platform, connection_type, hardware_concurrency, device_memory, user_id, last_path, hit_count, first_seen_at, last_seen_at"
          )
      ...
      return {
        ...
        weekly,
        monthly,
        devices: (devicesRes.data || []).map(mapDeviceRow),
