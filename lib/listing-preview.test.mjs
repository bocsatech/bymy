...
test("sanitizeListingPlainText kiszűri a Használtautó.hu / Belépés fejlécet", async () => {
  ...
  assert.equal(formatListingDisplayTitle("Használtautó.hu Belépés BMW X5"), "BMW X5");
  const { sanitizeListingFieldValue, composeVehicleTitle } = await import("./listing-preview.mjs");
  assert.equal(sanitizeListingFieldValue("Hiba!"), "");
  assert.equal(sanitizeListingFieldValue("Audi"), "Audi");
  assert.equal(composeVehicleTitle({ gyartmany: "A", modell: "", gyartasi_ev: "2017" }), "");
  assert.equal(composeVehicleTitle({ gyartmany: "BMW", modell: "X5", gyartasi_ev: "2017" }), "BMW X5 (2017)");
  assert.equal(
    composeVehicleTitle({
      gyartmany: "2022/10 , Benzin/elektromos (23468730)",
      modell: "",
      gyartasi_ev: "2022",
    }),
    ""
  ...
  assert.equal(sanitizeListingFieldValue("böngésző nem támogatja a JavaScript-et."), "");
  assert.equal(composeVehicleTitle({ gyartmany: "LAND", modell: "", gyartasi_ev: "2022" }), "Land Rover (2022)");
});
...
test("buildListingPreview elutasítja az évjárat+üzemanyag gyártmányt", () => {
  const preview = buildListingPreview(
    {
      hirdetes_cime: "2022/10 , Benzin/elektromos (23468730)",
      gyartmany: "2022/10 , Benzin/elektromos (23468730)",
      modell: "",
  ...
  assert.equal(preview.title, "Hirdetés #250");
  assert.equal(preview.filter.gyartmany, "");
});
