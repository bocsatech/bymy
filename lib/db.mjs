...
import { formDataToCells, cellsToFormData, FORM_FIELD_CATALOG, DROPPED_FORM_FIELD_KEYS } from "./form-field-catalog.mjs";
import {
...
import { normalizeFormVertical, resolveListingVertical } from "./listing-vertical.mjs";
import {
  ADMIN_LISTING_CELL_KEYS,
  mapListingRowForAdmin,
  filterAdminListings,
} from "./admin-listings.mjs";
...
function loadAdminCellsByListingIds(ids) {
  if (!ids.length) return new Map();
  const db = getDb();
  const idPh = ids.map(() => "?").join(",");
  const keyPh = ADMIN_LISTING_CELL_KEYS.map(() => "?").join(",");
  const rows = db
    .prepare(
      `SELECT listing_id, field_key, value
       FROM listing_cells
       WHERE listing_id IN (${idPh}) AND field_key IN (${keyPh})`
    )
    .all(...ids, ...ADMIN_LISTING_CELL_KEYS);
  const map = new Map();
  for (const row of rows) {
    if (!map.has(row.listing_id)) map.set(row.listing_id, []);
    map.get(row.listing_id).push(row);
...
export function listListingsForAdmin({ limit = 150, vertical = null, excludeVertical = null } = {}) {
  const needFilter = Boolean(vertical || excludeVertical);
  const fetchLimit = needFilter ? Math.min(Math.max(limit * 6, 300), 600) : limit;
  const rows = listListings({ limit: fetchLimit });
  const cellsById = loadAdminCellsByListingIds(rows.map((row) => row.id));
  const items = rows.map((row) => mapListingRowForAdmin(row, cellsById.get(row.id) ?? []));
  return filterAdminListings(items, { vertical, excludeVertical, limit });
}
...
export function listMyListings({ userId, limit = 200 } = {}) {
  ...
  const rows = listListingsWithPreview({ limit });
  if (!Number.isFinite(uid) || uid <= 0) return rows;
  return rows.filter((row) => Number(row.user_id) === uid);
}
