...
function parseTitle(html) {
  ...
  for (const match of String(html ?? "").matchAll(/<h1[^>]*>([\s\S]*?)<\/h1>/gi)) {
    candidates.push(cleanText(match[1].replace(/<[^>]+>/g, "")));
  }
  const ogTitle = html.match(/<meta[^>]*property="og:title"[^>]*content="([^"]+)"/i);
  if (ogTitle) candidates.push(cleanText(ogTitle[1]));
  const jsonName = html.match(/"name"\s*:\s*"([^"]{3,120})"/);
  if (jsonName) candidates.push(cleanText(jsonName[1]));
  const title = html.match(/<title[^>]*>([\s\S]*?)<\/title>/i);
  if (title) candidates.push(cleanText(title[1].replace(/\s*[|–-].*$/, "")));
