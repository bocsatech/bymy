...
function serverImagesRoot() {
  if (process.env.AUTOSWEB_IMAGES_PATH) return process.env.AUTOSWEB_IMAGES_PATH;
  return join(DATA_DIR, "images");
}

export function serverImageDir(kind) {
  return join(serverImagesRoot(), normalizeKind(kind));
}
...
function seedKindFromPublic(kind) {
  const dest = serverImageDir(kind);
  const src = join(PUBLIC_IMAGES, kind);
...
function migrateLegacyUploads(kind) {
  const dest = serverImageDir(kind);
  const candidates = [
    join(homedir(), ".autosweb", "uploads", "hero", kind),
    kind === "pkw" ? join(homedir(), ".autosweb", "uploads", "hero") : null,
  ].filter(Boolean);
  for (const src of candidates) {
    ...
    for (const name of names) {
      if (name.startsWith(".") || !/\.(png|jpe?g|webp)$/i.test(name)) continue;
      const from = join(src, name);
      const to = join(dest, name);
      if (!existsSync(to)) {
        try {
...
export function ensureServerImageDirs() {
  try {
    ensureDataDir();
    mkdirSync(serverImagesRoot(), { recursive: true });
    for (const kind of HERO_KINDS) {
      mkdirSync(serverImageDir(kind), { recursive: true });
      seedKindFromPublic(kind);
      migrateLegacyUploads(kind);
    }
  } catch {
    /* Vercel: a fájlrendszer lehet csak olvasható */
  }
  return serverImagesRoot();
}
...
async function saveUploadBuffer(kind, photo) {
  ...
  if (isSupabaseBackend()) {
    ...
    return { id, url: rewritePublicStorageUrl(data?.publicUrl || null) };
  }
  const dir = ensureUploadDir(k);
  const name = `${id}.${photo.ext}`;
  writeFileSync(join(dir, name), photo.buf);
  return { id, url: `/images/${k}/${name}` };
}

function listServerKindImages(kind) {
  const k = normalizeKind(kind);
  ensureServerImageDirs();
  const dir = serverImageDir(k);
...
function kindPayload(kind, state) {
  ...
  const hardcoded = HERO_PRESETS[k] || [];
  const known = new Set([
    ...hardcoded.map((p) => p.url),
    ...slice.uploads.map((u) => u.url),
  ]);
  const extra = listServerKindImages(k)
    .filter((item) => !known.has(item.url) && !/^hero-\d/.test(item.name))
    .map(({ id, label, url }) => ({ id, label, url }));
  return {
    kind: k,
    activeUrl: slice.activeUrl,
    presets: [...hardcoded, ...extra],
...
export function resolveHeroUploadFile(urlPath) {
  ...
  if (parts.length === 2 && HERO_KINDS.includes(parts[0])) {
    ...
    const legacyKind = join(homedir(), ".autosweb", "uploads", "hero", kind, name);
    return existsSync(legacyKind) ? legacyKind : null;
  }
  if (parts.length === 1) {
    const name = parts[0];
    if (!name || name.includes("..") || name.includes("\\")) return null;
...
/** Szerver oldali fájl: /images/pkw/… és /images/lkw/… → data/images/{kind}/ */
export function resolveServerImageFile(urlPath) {
  const rel = String(urlPath ?? "").replace(/^\//, "");
  const match = rel.match(/^images\/(pkw|lkw)\/([^/]+)$/);
  if (!match) return null;
  const [, kind, name] = match;
  if (!name || name.includes("..") || name.includes("\\")) return null;
  ensureServerImageDirs();
  const full = join(serverImageDir(kind), name);
