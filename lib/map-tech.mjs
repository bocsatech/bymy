...
import { transformFieldValue } from "./field-key-map.mjs";
import { splitEquipmentAndEgyebInfo } from "./egyeb-info-catalog.mjs";
...
export function mapSebessegvalto(value, hints = "") {
  ...
  if (!v) return "";
  // HA részletes érték (tiptronic / fokozat) maradjon — ne vágjuk „Automata”-ra
  if (raw && /(tiptronic|fokozat|sebességváltó|sebessegvalto)/i.test(raw) && raw.length > 12) {
    return raw;
  }
...
export function mapHengerElrendezes(value) {
  ...
  if (v.includes("sor")) return cleanText(value) || "Soros";
  return cleanText(value);
}
...
export function mapKlima(value, badges = [], extraText = "") {
  ...
  if (/\btobbzon\w*\b|\b[3-9]\s*zonas?\b|\b[3-9]\s*zona\b/.test(v)) return "digitális többzónás klíma";
  if (/\bketzon\w*\b|\b2\s*zonas?\b|\b2\s*zona\b/.test(v)) return "digitális kétzónás klíma";
...
export function applyExtrakFields(data, parsed, m, badges = []) {
  ...
  // Előbb a kanonikus katalógus-találatok, aztán a nyers HA tételek (duplikátum nélkül)
  const mergedEquipment = [];
  const seen = new Set();
  const pushEquip = (raw) => {
    const text = cleanText(raw);
    if (!text || text.length < 2 || text.length > 90) return;
    if (/^(beltér|műszaki|kültér|multimédia|egyéb|felszereltség|nincs)$/i.test(text)) return;
    const key = normalizeKey(text);
    if (!key || seen.has(key)) return;
    seen.add(key);
    mergedEquipment.push(text);
  };
  for (const item of equipment) pushEquip(item);
  for (const item of badges) pushEquip(item);

  if (mergedEquipment.length) {
    const split = splitEquipmentAndEgyebInfo(mergedEquipment);
    if (split.felszereltseg.length) data.felszereltseg = split.felszereltseg;
    if (split.egyeb_info.length) {
      data.egyeb_info = [...new Set([...(data.egyeb_info ?? []), ...split.egyeb_info])];
    }
  }
...
export function applyMuszakiFields(data, parsed, m, badges = []) {
  ...
  if (!data.szin) data.szin = transformFieldValue("szin", pickValue(merged, ["szín", "szin", "külső szín"]) || "");
  if (!data.karpit1) {
    data.karpit1 = transformFieldValue(
      "karpit1",
      pickValue(merged, ["kárpit színe (1)", "karpit szine (1)", "kárpit színe", "karpit szine"]) || ""
    );
  }
