...
test("hirdetésnézet a feltöltött autóadatokból, magyar címkékkel", () => {
  ...
  assert.ok(view.vehicleSpecs.some((row) => row.label === "Sebességváltó" && row.value.includes("automata")));
  ...
  assert.ok(view.vehicleSpecs.some((row) => row.label === "Össztömeg" && /2410/.test(row.value)));
  assert.ok(view.vehicleSpecs.some((row) => row.label === "Klíma" && /digitális klíma/i.test(row.value)));
