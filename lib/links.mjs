...
/** Nyilvános hirdetés: /kategoria/.../slug-12345 (személyautó, kishaszonjármű, …). */
const LISTING_PATH_RE = /\/[^/?#]+\/.+-\d{5,}\/?$/i;
const LISTING_URL_RE =
  /https?:\/\/(?:www\.)?hasznaltauto\.hu\/[^/?#\s]+\/[^?\s#]*-\d{5,}/gi;
...
export function extractListingIdFromUrl(input) {
  const raw = String(input ?? "").trim();
  if (!raw) return "";
  try {
    const url = new URL(raw);
    const gyors = url.pathname.match(/\/gyorsnezet\/[^/]+\/(\d{5,})/i);
    if (gyors) return gyors[1];
    const slug = url.pathname.match(/-(\d{5,})$/);
...
export function publicListingUrlFromId(id) {
  const n = String(id ?? "").replace(/\D/g, "");
  if (n.length < 5) return "";
  return `https://www.hasznaltauto.hu/szemelyauto/import-${n}`;
}

export function isAdminListingPageUrl(input) {
  try {
    const url = new URL(String(input ?? "").trim());
    const host = url.hostname.replace(/^www\./, "").toLowerCase();
    if (!host.includes("hasznaltauto.hu")) return false;
    if (host.startsWith("admin.")) return true;
    return /\/gyorsnezet\//i.test(url.pathname);
  } catch {
    return false;
  }
}

export function extractCanonicalPublicUrl(html) {
  const source = String(html ?? "");
  const og = source.match(/property=["']og:url["'][^>]*content=["']([^"']+)["']/i)
    || source.match(/content=["']([^"']+)["'][^>]*property=["']og:url["']/i);
  if (og?.[1] && isListingUrl(og[1])) return normalizeListingHref(og[1], og[1]);
  const canon = source.match(/rel=["']canonical["'][^>]*href=["']([^"']+)["']/i)
    || source.match(/href=["']([^"']+)["'][^>]*rel=["']canonical["']/i);
  if (canon?.[1] && isListingUrl(canon[1])) return normalizeListingHref(canon[1], canon[1]);
  return "";
}

export function extractListingRefsFromHtml(html, baseUrl) {
  const source = String(html ?? "");
  ...
  const add = (id, extra = {}) => {
    const n = String(id ?? "").replace(/\D/g, "");
    if (n.length < 5) return;
    const prev = byId.get(n) || { id: n };
    byId.set(n, {
      ...prev,
      ...extra,
      id: n,
      adminUrl: extra.adminUrl || prev.adminUrl || `https://admin.hasznaltauto.hu/gyorsnezet/szemelyauto/${n}`,
      publicUrl: extra.publicUrl || prev.publicUrl || publicListingUrlFromId(n),
    });
  };

  for (const m of source.matchAll(/\/gyorsnezet\/[^/"'\s]+\/(\d{5,})/gi)) {
    add(m[1], { adminUrl: `https://admin.hasznaltauto.hu/gyorsnezet/szemelyauto/${m[1]}` });
  }
  for (const url of extractListingLinksFromHtml(source, base || "https://www.hasznaltauto.hu/")) {
    add(extractListingIdFromUrl(url), { publicUrl: url });
  }
  for (const m of source.matchAll(/data-(?:id|hirdetesid|adid)=["'](\d{5,})["']/gi)) {
    add(m[1]);
  }
  if (isAdminListingPageUrl(base) || isListingUrl(base)) {
    add(extractListingIdFromUrl(base), {
      adminUrl: isAdminListingPageUrl(base) ? base : undefined,
      publicUrl: isListingUrl(base) ? base : undefined,
    });
  }
  return [...byId.values()];
}
