...
test("sanitizeListingList: feed vékonyítás", () => {
  ...
  assert.equal(out[0].user_id, undefined);
  assert.deepEqual(out[0].preview.imageUrls, ["a.jpg"]);
  assert.ok(out[0].preview.leiras.length <= 281);
  assert.equal(out[0].form.telefon, undefined);
  assert.equal(out[0].form.gyartmany, "BMW");
  assert.equal(out[0].form.felszereltseg.length, 40);
  assert.ok(out[0].form.leiras.length <= 401);
});
