...
export async function importFromClient(options = {}) {
  ...
  for (const url of urls) {
    try {
      ...
      if (!html) {
        throw new Error("Hiányzik a hirdetés HTML-je. Nyisd meg a Módosítás / teljes hirdetés oldalt, majd importálj újra.");
      }
      const entryMeta = {
      ...
      if (autoSave) {
        // Import: nincs listing-kvóta. Duplikátum → frissítés.
        const incoming = { ...(item.form || {}), fo_kep: item.form?.fo_kep || item.imageUrl || "" };
        const form = existing?.form
          ? {
              ...existing.form,
              ...Object.fromEntries(Object.entries(incoming).filter(([, value]) => value !== "" && value != null)),
            }
          : incoming;
