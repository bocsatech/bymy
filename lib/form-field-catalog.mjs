...
export const FORM_FIELD_CATALOG = [
  ...
  { field_key: "jelenlegi_akkukapacitas", label: "Jelenlegi akkukapacitás", step: 2 },
  { field_key: "ac_toltesi_teljesitmeny", label: "AC töltési teljesítmény", step: 2 },
  { field_key: "dc_toltesi_teljesitmeny", label: "DC töltési teljesítmény", step: 2 },
  { field_key: "ac_tolto_csatlakozas", label: "AC töltőcsatlakozó típusa", step: 2 },
  { field_key: "dc_tolto_csatlakozas", label: "DC töltőcsatlakozó típusa", step: 2 },
  { field_key: "hatotav", label: "WLTP hatótáv", step: 2 },
  { field_key: "autopalya_hatotav", label: "Autópálya hatótáv", step: 2 },
  { field_key: "teli_hatotav", label: "Téli hatótáv", step: 2 },
  { field_key: "villamtoltes", label: "Villámtöltés", step: 2 },
  { field_key: "zold_rendszam", label: "Zöld rendszám", step: 2 },
  ...
  { field_key: "leiras", label: "Leírás", step: 3 },
  ...
  { field_key: "photo_overlay_template_id", label: "Kép sablon", step: 4 },
  { field_key: "photo_overlay_base_url", label: "Kép sablon alapkép", step: 4 },
  { field_key: "promo_kiemelt", label: "Kiemelés", step: 9 },
  { field_key: "promo_top_ajanlat", label: "TOP ajánlat", step: 9 },
...
function mergeFieldCatalogs(...lists) {
  ...
  for (const list of lists) {
    for (const def of list) {
      if (!def?.field_key || seen.has(def.field_key)) continue;
      seen.add(def.field_key);
      out.push(def);
    }
  }
  return out;
...
export const ALL_FORM_FIELD_DEFS = mergeFieldCatalogs(FORM_FIELD_CATALOG, ingatlanFormFieldCatalog());

const LABEL_BY_KEY = new Map(ALL_FORM_FIELD_DEFS.map((f) => [f.field_key, f.label]));
...
export function formDataToCells(formData) {
  ...
  for (const def of ALL_FORM_FIELD_DEFS) {
