...
test("getImporterListingAddressFromProfile: magán utca is jön", () => {
  const addr = getImporterListingAddressFromProfile({
    accountType: "private",
    street: "Petőfi u. 9.",
    postalCode: "9021",
    city: "Győr",
  });
  assert.equal(addr.street, "Petőfi u. 9.");
  assert.equal(addr.postalCode, "9021");
  assert.equal(addr.city, "Győr");
});

test("applyImporterProfileToListingForm: HA adat helyett importáló profil", () => {
  const form = {
    megtekintesi_cim: "HA Kereskedés utca 1.",
    iranyitoszam: "1111",
    telepules: "Debrecen",
    telefon1_korzet: "70",
    telefon1_szam: "999 9999",
  };
  applyImporterProfileToListingForm(form, {
    accountType: "business",
    companyStreet: "Ipari park 3.",
    companyPostalCode: "2040",
    companyCity: "Budaörs",
  ...
  assert.equal(form.megtekintesi_cim, "Ipari park 3.");
  assert.equal(form.iranyitoszam, "2040");
  assert.equal(form.telepules, "Budaörs");
  assert.equal(form.megye, "Pest");
  assert.equal(form.telefon1_orszag, "+36");
  assert.equal(form.telefon1_korzet, "20");
  assert.equal(form.telefon1_szam, "123 4567");
});
