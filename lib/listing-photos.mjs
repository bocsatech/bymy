...
import { isFilesystemImageStorage } from "./filesystem-image-storage.mjs";
import { uploadOptimizedImage } from "./supabase/image-storage.mjs";
...
async function saveToSupabase(listingId, photo, index) {
  ...
  return rewritePublicStorageUrl(data?.publicUrl || null);
}
  ...
  for (let i = 0; i < list.length; i += 1) {
    ...
    if (isFilesystemImageStorage()) {
      const uploaded = await uploadOptimizedImage({
        fileBuffer: photo.buf,
        bucket: "listing-images",
        folder: String(id),
        fileName: `photo-${i}-${randomBytes(4).toString("hex")}`,
        options: { maxBytes: MAX_BYTES, minWidth: 320, minHeight: 240 },
      });
      url = uploaded.publicUrl;
    } else if (isSupabaseBackend()) {
      url = await saveToSupabase(id, photo, i);
    } else {
      url = saveLocal(id, photo, i);
    }
