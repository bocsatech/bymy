...
test("kereskedő: cég tevékenység Autó + Teherautó — mindkét vertical mehet", () => {
  const user = {
    id: 3,
    profile: { accountType: "business", companyActivities: ["auto", "teherauto"] },
  };
  const existing = [{ form: { hirdetes_vertical: "teher", hirdetes_alkategoria: "kisteher" } }];
  const autoOk = assertCanCreateListing({
    user,
    formData: { hirdetes_vertical: "auto", hirdetes_alkategoria: "szemelyauto" },
    existingListings: existing,
  });
  assert.equal(autoOk.ok, true);
  const teherOk = assertCanCreateListing({
    user,
    formData: { hirdetes_vertical: "teher", hirdetes_alkategoria: "kisteher" },
    existingListings: [{ form: { hirdetes_vertical: "auto" } }],
  });
  assert.equal(teherOk.ok, true);
  const blocked = assertCanCreateListing({
    user,
    formData: { hirdetes_vertical: "ingatlan" },
    existingListings: [],
  ...
  assert.equal(blocked.ok, false);
  assert.equal(blocked.code, "VERTICAL_NOT_ALLOWED");
});

test("kereskedő: tevékenység nélkül legacy zár az első hirdetés verticaljára", () => {
  const user = { id: 4, profile: { accountType: "dealer", companyActivities: [] } };
  const r = assertBusinessVerticalAllowed(user, "auto", [
    { form: { hirdetes_vertical: "teher", hirdetes_alkategoria: "kisteher" } },
  ]);
  assert.equal(r.ok, false);
  assert.equal(r.code, "VERTICAL_LOCKED");
});
