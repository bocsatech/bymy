...
function accountFallbackPhone(user = {}) {
  const profile = user.profile && typeof user.profile === "object" ? user.profile : {};
  return text(profile.companyPhone || profile.phone, 40);
}

function accountFallbackAreas(user = {}) {
  const profile = user.profile && typeof user.profile === "object" ? user.profile : {};
  return text(profile.companyCity || profile.city, 1000);
}
...
function normalizeProfile(input = {}, user = {}) {
  ...
  const phone = normalizePhone(input.phone || accountFallbackPhone(user));
  ...
  const serviceAreas = text(input.serviceAreas || input.service_areas || accountFallbackAreas(user), 1000);
