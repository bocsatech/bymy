...
export function getPostalCode(postalCode) {
  if (useSeedPostalStore()) {
    return lookupPostalCodeFromSeed(postalCode);
  }
...
/** Település → irányítószám (pontos név). Több IRSZ esetén a legkisebb. */
export function getPostalCodeByCity(cityName) {
  const city = String(cityName ?? "").trim();
  if (!city) return null;
  if (useSeedPostalStore()) {
    return lookupCityFromSeed(city);
  }
  const db = getDb();
  const row = db
    .prepare(
      `SELECT postal_code, city, lat, lon
       FROM postal_codes
...
export function listPostalCities() {
  if (useSeedPostalStore()) {
    return listPostalCitiesFromSeed();
  }
  const db = getDb();
  return db
    .prepare(
...
export function getPartnerRecommendations(postalCodeInput, options = {}) {
  ...
  const vertical = normalizePartnerVertical(options.vertical) ?? "auto";
  const catalog = categoriesForVertical(vertical);
