...
export function parseBodyTextAttributes(text) {
  ...
  const inlinePatterns = [
    ...
    ["Kárpit színe (1)", /\bKárpit színe\s*\(?1\)?\s*[:\n]\s*([^\n]{2,40})/gi],
  ...
  for (let i = 0; i < lines.length - 1; i += 1) {
    ...
    // Már „Címke: érték” egy sorban — ne párosítsuk a következő sorral.
  ...
  // Egy soros „Címke:érték” / „Címke: érték” (HA gyakran szóköz nélkül).
