...
/** Live desk feladás: egy master elrendezés, többi autótípus ezt örökli. */
export const DESK_POSTING_LAYOUT_MASTER = "szemelyauto";

export const DESK_POSTING_LAYOUT_ALIASES = new Set([
  "leasing",
  "berauto",
  "lakokocsi",
  "kisteher",
...
const SEARCH_ONLY_FIELD_KEYS = new Set(["keresesi_korzet"]);
...
export function isDeskPostingLayoutAlias(category) {
  return DESK_POSTING_LAYOUT_ALIASES.has(normalizeLayoutCategory(category));
}

export function deskPostingLayoutSourceCategory(category) {
  const id = normalizeLayoutCategory(category);
  return isDeskPostingLayoutAlias(id) ? DESK_POSTING_LAYOUT_MASTER : id;
}

export function isVehiclePostingLayoutCategory(category) {
  const id = normalizeLayoutCategory(category);
  if (isSearchLayoutCategory(id) || id === "ingatlan") return false;
  return BY_ID.has(id);
}

export function vehiclePostingPreviewPath(category) {
  const id = normalizeLayoutCategory(category);
  const meta = BY_ID.get(id);
  if (!meta || isSearchLayoutCategory(id) || id === "ingatlan") return "/hirdetesfeladas.html?start=1";
  const vertical = encodeURIComponent(meta.vertical || "auto");
  const subtype = encodeURIComponent(meta.subtype || id);
  return `/hirdetesfeladas.html?vertical=${vertical}&subtype=${subtype}&start=1`;
}
...
function vehicleCatalog({ includeKisteher = false, includeSearchOnly = false } = {}) {
  return FORM_FIELD_CATALOG.filter((field) => {
    if (SEARCH_ONLY_FIELD_KEYS.has(field.field_key)) return includeSearchOnly;
...
export function catalogForLayoutCategory(category) {
  const id = normalizeLayoutCategory(category);
  if (id === "teherauto-search") {
    return vehicleCatalog({ includeKisteher: true, includeSearchOnly: true });
  }
  if (id === "kisteher") {
    return vehicleCatalog({ includeKisteher: true, includeSearchOnly: false });
  }
  if (id === "szemelyauto-search") {
    return vehicleCatalog({ includeKisteher: false, includeSearchOnly: true });
  }
  if (id === "ingatlan") return ingatlanCatalog();
  return vehicleCatalog({ includeKisteher: false, includeSearchOnly: false });
}
