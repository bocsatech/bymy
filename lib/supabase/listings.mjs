...
async function fetchListingRows({ limit = 50, status = null } = {}) {
  let q = sb()
    .from("listings")
    .select(
...
const FEED_CACHE_TTL_MS = Math.max(5_000, Number(process.env.LISTINGS_FEED_CACHE_TTL_MS) || 20_000);
...
async function listListingsWithPreviewUncached({ limit = 50, status = null, vertical = null } = {}) {
  ...
  // Vertical szűréshez elég sok ID + 2 cella/listing; a teljes cella-csomag csak a végső limithoz.
