...
test("sanitizeListingPlainText kiszűri a Használtautó.hu / Belépés fejlécet", async () => {
  ...
  assert.equal(
    composeVehicleTitle({
      gyartmany: "NISSAN",
      modell: "",
      tipus: "Qashqai 1.6 dCi",
      gyartasi_ev: "2013",
    }),
    "NISSAN Qashqai 1.6 dCi (2013)"
  );
  assert.equal(
    composeVehicleTitle({
      gyartmany: "ABARTH",
      modell: "500",
      tipus: "500 Coupe 1.4 TJet 140",
      gyartasi_ev: "2016",
    }),
    "ABARTH 500 Coupe 1.4 TJet 140 (2016)"
  );
  assert.equal(
    composeVehicleTitle({ gyartmany: "JAGUAR", modell: "F-Pace", tipus: "—", gyartasi_ev: "2021" }),
    "JAGUAR F-Pace (2021)"
  );
});
