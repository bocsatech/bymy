...
export function seedPostalCodesIfEmpty(db) {
  ...
  const count = Number(db.prepare("SELECT COUNT(*) AS n FROM postal_codes").get().n) || 0;
  // Régi, rövid seed (pl. ~250 sor) → pótoljuk a teljes listából.
  if (count >= rows.length) return { inserted: 0, total: count };
  ...
  const total = Number(db.prepare("SELECT COUNT(*) AS n FROM postal_codes").get().n) || 0;
