...
import { listingImageUrlForConversation, listingImageUrlsByIds } from "./messaging-listing-image.mjs";
import {
...
async function publicConversation(db, conv, userId, imageMap = null) {
  ...
  const listingId = Number(conv.listing_id);
  const imageUrl = imageMap?.get(listingId) || (await listingImageUrlForConversation(conv.listing_id));
  return {
    id: conv.id,
    listing: {
      id: conv.listing_id,
...
function purgeEmptyConversationRows(db, rows) {
  if (!rows.length) return [];
  const ids = rows.map((row) => row.id);
  const placeholders = ids.map(() => "?").join(",");
  const withMessages = new Set(
    db
      .prepare(`SELECT DISTINCT conversation_id FROM messages WHERE conversation_id IN (${placeholders})`)
      .all(...ids)
      .map((row) => row.conversation_id)
  );
  const emptyIds = ids.filter((id) => !withMessages.has(id));
  if (emptyIds.length) {
    db.prepare(`DELETE FROM conversations WHERE id IN (${emptyIds.map(() => "?").join(",")})`).run(...emptyIds);
  }
  return rows.filter((row) => withMessages.has(row.id));
}
...
export async function listConversations(userId) {
  ...
  initMessagingSchema();
  const db = getDb();
  const rows = purgeEmptyConversationRows(
    db,
    db
      .prepare(
  ...
  const imageMap = await listingImageUrlsByIds(rows.map((row) => row.listing_id));
  return Promise.all(rows.map((c) => publicConversation(db, c, userId, imageMap)));
}

function resolveSellerFromListing(db, listingId) {
  const id = Number(listingId);
  if (!Number.isFinite(id) || id <= 0) return 0;
  const cell = db
    .prepare(
      `SELECT value FROM listing_cells WHERE listing_id = ? AND field_key = 'owner_user_id' LIMIT 1`
    )
    .get(id);
  const owner = Number(cell?.value);
  return Number.isFinite(owner) && owner > 0 ? owner : 0;
}

function resolveConversationPartiesSqlite(db, userId, input) {
  const listingId = String(input.listing_id ?? input.listingId ?? "").trim();
  if (!listingId) {
    const err = new Error("A hirdetés azonosítója kötelező.");
    err.status = 400;
    throw err;
  }
  ...
  if (!Number.isFinite(sellerId) || sellerId <= 0) {
    sellerId = resolveSellerFromListing(db, listingId);
  }
  if (!sellerId) {
    const sellerEmail = String(input.seller_email ?? input.sellerEmail ?? "")
      .trim()
      .toLowerCase();
    if (sellerEmail) {
  ...
  if (!sellerId) {
    const err = new Error("A hirdetésnek nincs tulajdonosa — üzenet nem indítható.");
    err.status = 400;
    throw err;
  }
  if (sellerId === userId) {
  ...
  return {
    listingId,
    sellerId,
    title: String(input.listing_title ?? input.listingTitle ?? "").trim(),
    priceLabel: String(input.listing_price_label ?? input.listingPriceLabel ?? ""),
    meta: String(input.listing_meta ?? input.listingMeta ?? ""),
    code: String(input.listing_code ?? input.listingCode ?? `AEA-${listingId}`).trim(),
  };
}

function findExistingConversationRowSqlite(db, listingId, buyerId, sellerId) {
  return (
    db
      .prepare(
        `SELECT * FROM conversations
      ...
      .get(listingId, buyerId, sellerId) ?? null
  );
}

function purgeEmptyConversationRowSqlite(db, conv) {
  if (!conv) return true;
  const count = db
...
function insertConversationRowSqlite(db, userId, parties) {
  const info = db
    .prepare(
...
export async function findConversationForListing(userId, input) {
  if (isSupabaseBackend()) return sbMessaging.findConversationForListing(userId, input);
  initMessagingSchema();
  const db = getDb();
  const parties = resolveConversationPartiesSqlite(db, userId, input);
  let existing = findExistingConversationRowSqlite(db, parties.listingId, userId, parties.sellerId);
  if (!existing) return null;
  if (purgeEmptyConversationRowSqlite(db, existing)) return null;
  return publicConversation(db, existing, userId);
}

export async function startConversationWithInitial(userId, input) {
  if (isSupabaseBackend()) return sbMessaging.startConversationWithInitial(userId, input);
  initMessagingSchema();
  const db = getDb();
  const parties = resolveConversationPartiesSqlite(db, userId, input);
  if (!parties.title) {
    const err = new Error("A hirdetés címe kötelező.");
    err.status = 400;
    throw err;
  }
  const text = String(input.initial_body ?? input.message ?? "").trim();
  ...
  let row = findExistingConversationRowSqlite(db, parties.listingId, userId, parties.sellerId);
  if (row && purgeEmptyConversationRowSqlite(db, row)) row = null;
  if (!row) row = insertConversationRowSqlite(db, userId, parties);

  await sendMessage(userId, row.id, { body: text, attachment });
  const fresh = db.prepare("SELECT * FROM conversations WHERE id = ?").get(row.id);
  return publicConversation(db, fresh ?? row, userId);
}
...
export async function startConversation(userId, input) {
  ...
  const text = String(input.initial_body ?? input.message ?? "").trim();
  if (text || input.attachment) return startConversationWithInitial(userId, input);
  return findConversationForListing(userId, input);
}
...
export async function listMessages(userId, conversationId) {
  ...
  const rows = db
    ...
    .all(conversationId);
  return {
    conversation: await publicConversation(db, conv, userId),
...
export async function handleMessagesApi(req, res, pathname) {
  ...
  try {
    ...
    if (pathname === "/api/messages/conversations/for-listing" && req.method === "GET") {
      const user = await requireUserAsync(req);
      const url = new URL(req.url || "/", "http://localhost");
      const input = {
        listing_id: url.searchParams.get("listing_id") || "",
        listing_title: url.searchParams.get("title") || "",
        listing_price_label: url.searchParams.get("price") || "",
        listing_meta: url.searchParams.get("meta") || "",
        listing_code: url.searchParams.get("code") || "",
        seller_id: url.searchParams.get("seller_id") || "",
      };
      sendJson(res, 200, {
        ok: true,
        conversation: await findConversationForListing(user.id, input),
      });
      return true;
    }
    ...
    if (pathname === "/api/messages/conversations" && req.method === "POST") {
      ...
      const initialBody = String(body.initial_body ?? body.message ?? "").trim();
      if (initialBody || body.attachment) {
        sendJson(res, 201, {
          ok: true,
          conversation: await startConversationWithInitial(user.id, body),
        });
        return true;
      }
      const existing = await findConversationForListing(user.id, body);
      if (!existing) {
        sendJson(res, 404, {
          ok: false,
          code: "NO_CONVERSATION",
          error: "Még nincs beszélgetés ehhez a hirdetéshez.",
        });
        return true;
      }
      sendJson(res, 200, { ok: true, conversation: existing });
      return true;
    }
    ...
    if (convReport && req.method === "POST") {
      const user = await requireUserAsync(req);
      const body = await readJsonBody(req);
      const conversationId = Number(convReport[1]);
      // Ellenőrizzük, hogy a user része a beszélgetésnek (listMessages dob, ha nem)
      await listMessages(user.id, conversationId);
      const reason = String(body.reason ?? body.message ?? "").trim().slice(0, 500);
      console.warn(
        `Message report conversation=${conversationId} by=${user.id} reason=${reason || "(üres)"}`
      );
