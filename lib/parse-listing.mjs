import { extractFelszereltsegFromHtml } from "./ha-equipment-extract.mjs";
  ...
  for (const key of keys) {
    ...
    for (const [entryKey, entryValue] of Object.entries(map)) {
      if (normalizeKey(entryKey) === normalized && cleanText(entryValue)) {
        ...
        // Évjárat / méret értékek érvényesek — a chrome-szűrő a címsorokra való
        if (isListingSiteChromeLine(value) && !/^(19|20)\d{2}(?:\/\d{1,2})?$/.test(value) && !/^[\d\s./]+(?:\s*(?:km|ft|cm³|kw|le|kg))?$/i.test(value)) {
          continue;
        }
        return value;
      }
...
/** HA oldalakon gyakran egy sorban ragad: „Fekete (metál) Saját tömeg: 1 700 kg …” */
const CHAINED_HA_LABEL_RE =
  /\s+(?=(?:Saját tömeg|Össztömeg|Kárpit színe(?:\s*\(\d+\))?|Csomagtartó|Hengerűrtartalom|Teljesítmény|Sebességváltó|Hajtás|Üzemanyag|Állapot|Évjárat|Futásteljesítmény|Vételár|Ajtók száma|Szállítható|Klíma(?:\s+fajtája)?|Tető|Nyári gumi|Téli gumi|Okmányok|Szín|Saját\s+tömeg|Co2)\s*:)/i;

export function splitChainedHaValue(value) {
  const v = cleanText(value);
  if (!v) return "";
  const cut = v.split(CHAINED_HA_LABEL_RE)[0] || v;
  return cleanText(cut.replace(/\s+(?:Saját tömeg|Össztömeg|Kárpit)\s*$/i, ""));
}

function hasNestedHaLabels(value) {
  return CHAINED_HA_LABEL_RE.test(` ${cleanText(value)}`);
}
...
function attributeValueScore(value) {
  ...
  if (hasNestedHaLabels(v)) return -500;
  // Tiszta mezők: a rövidebb, nem összefolyt érték a jobb (ne a hosszabb nyerjen).
...
export function mergeAttributeMaps(...maps) {
  ...
  for (const map of maps) {
    ...
    for (const [rawKey, rawValue] of Object.entries(map)) {
      ...
      const value = splitChainedHaValue(rawValue);
...
function addAttributePair(map, rawKey, rawValue) {
  const key = cleanText(String(rawKey).replace(/:$/, ""));
  let value = splitChainedHaValue(cellText(rawValue));
...
function parseAttributesTable(html) {
  ...
  // Admin gyorsnézet: class nélküli / változó táblázatok — ne maradjon ki Állapot, Kivitel, stb.
  if (Object.keys(map).length < 6) {
    for (const tableMatch of String(html ?? "").matchAll(/<table[^>]*>([\s\S]*?)<\/table>/gi)) {
      parseTableRows(tableMatch[1], map);
    }
  }
...
export function parseBodyTextAttributes(text) {
  ...
  const inlinePatterns = [
    ...
    ["Szín", /\bSzín\s*[:\n]\s*([^\n:]{2,40}?)(?=\s+(?:Saját tömeg|Össztömeg|Kárpit|Csomagtartó)\s*:|$|\n)/gi],
    ...
    ["Klíma fajtája", /\bKlíma fajtája\s*[:\n]\s*([^\n]{2,40})/gi],
    [
      "Kárpit színe (1)",
      /\bKárpit színe\s*\(?1\)?\s*[:\n]\s*([^\n:]{2,40}?)(?=\s+(?:Saját tömeg|Össztömeg|Csomagtartó|Szín)\s*:|$|\n)/gi,
    ],
    ["Saját tömeg", /\bSaját tömeg\s*[:\n]\s*([\d\s.]+\s*kg)/gi],
    ["Össztömeg", /\bÖssztömeg\s*[:\n]\s*([\d\s.]+\s*kg)/gi],
    ["Ajtók száma", /\bAjtók száma\s*[:\n]\s*(\d+)/gi],
    ["Szállítható személyek száma", /\bSzállítható szem\.?\s*száma\s*[:\n]\s*([^\n]{1,20})/gi],
  ];
  ...
  for (let i = 0; i < lines.length - 1; i += 1) {
    ...
    if (/^.{2,50}:\s*.+/.test(line) || /^.{2,50}:\S/.test(line)) continue;
  ...
  for (const line of lines) {
    const match = line.match(/^(.{2,55}?)\s*:\s*(.+)$/);
    if (!match) continue;
    const label = match[1].trim();
    const value = match[2].trim();
    if (!label || !value || value.length > 120) continue;
    if (labelHints.test(normalizeKey(label)) || /klima|karpit|gumi|okmany|szin|ajto|szem/i.test(normalizeKey(label))) {
      addAttributePair(map, label, value);
    }
  }
...
function parseEquipmentFromHtml(html) {
  return extractFelszereltsegFromHtml(html);
}
