...
export async function getIngatlanWheelSchema() {
  await initLevel1();
  const value = await readKv(INGATLAN_WHEEL_SCHEMA_KEY);
  if (value == null || value === "") return defaultIngatlanWheelSchema();
  try {
    return normalizeIngatlanWheelSchema(JSON.parse(value));
  } catch {
    return defaultIngatlanWheelSchema();
  }
}

export async function saveIngatlanWheelSchema(schema) {
  await initLevel1();
  const stored = normalizeIngatlanWheelSchema(schema && typeof schema === "object" ? schema : {});
  await writeKv(INGATLAN_WHEEL_SCHEMA_KEY, JSON.stringify(stored));
  return stored;
}
