...
async function ensureBootstrap() {
  ...
  const password = String(process.env.LEVEL1_BOOTSTRAP_PASSWORD ?? "");
...
export async function startLevel1Login(username, password) {
  ...
  if (!verifyPassword(pass, admin.password_salt, admin.password_hash)) {
  ...
  await insertOtp(user, tokenHash(code), expiresAt);
...
export async function getFormLayout() {
  ...
  if (isSupabaseBackend()) {
    ...
    if (error) throw error;
    if (!data?.value) return defaultFormLayout();
    try {
  ...
  if (!row?.value) return defaultFormLayout();
  try {
