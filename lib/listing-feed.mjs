...
export async function countListingFeed(sb, { status = null, vertical = null } = {}) {
  ...
  let q = sb.from("listing_feed").select("listing_id", { count: "exact", head: true });
...
export async function listListingFeed(
  sb,
  { limit = 50, offset = 0, status = null, vertical = null } = {}
) {
  if (!(await listingFeedTableExists(sb))) return null;
  const want = String(vertical ?? "")
    .trim()
