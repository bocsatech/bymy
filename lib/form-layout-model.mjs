...
import { catalogForLayoutCategory, normalizeLayoutCategory } from "./ad-form-layout-categories.mjs";
...
function defaultsForStep(step, catalog) {
...
/**
 * @param {object} layout
 * @param {{ category?: string, catalog?: Array }=} options
 */
export function normalizeFormLayout(layout, options = {}) {
  const incoming = layout && typeof layout === "object" ? layout : {};
  const category = normalizeLayoutCategory(options.category || incoming.category || "szemelyauto");
  const catalog = Array.isArray(options.catalog)
    ? options.catalog
    : catalogForLayoutCategory(category);
  const byKey = new Map(
    (Array.isArray(incoming.cells) ? incoming.cells : []).map((cell) => [String(cell.field_key || ""), cell])
  );
  const stepDefaults = new Map();
  for (const field of catalog) {
    if (!stepDefaults.has(field.step)) stepDefaults.set(field.step, defaultsForStep(field.step, catalog));
  }
  const cells = catalog.map((field, index) => {
  ...
  return { version: 2, category, cells };
}

/** @deprecated use normalizeFormLayout with category */
export function normalizeFormLayoutLegacy(layout) {
  return normalizeFormLayout(layout, { catalog: FORM_FIELD_CATALOG, category: "szemelyauto" });
}
