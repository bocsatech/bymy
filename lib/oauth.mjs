...
function apexHost(hostname) {
  return hostWithoutPort(hostname).replace(/^www\./i, "");
}

/** OAuth redirect_uri: aktuális host, kanonikus PUBLIC_BASE_URL (www → apex), ne Vercel env. */
export function publicBaseUrlFromRequest(req, cfg = loadOAuthConfig()) {
  const cfgBase = String(cfg?.publicBaseUrl || "").trim().replace(/\/$/, "");
  if (req?.headers) {
    const forwardedHost = req.headers["x-forwarded-host"];
    const host = hostWithoutPort(
      forwardedHost ? String(forwardedHost).split(",")[0].trim() : String(req.headers.host || "").trim()
    );
    if (host && !/^127\.0\.0\.1$/i.test(host) && !/^localhost$/i.test(host)) {
      if (cfgBase) {
        try {
          const canon = new URL(cfgBase);
          if (apexHost(host) === apexHost(canon.hostname)) {
            return canon.origin;
          }
        } catch {
  ...
  return cfgBase || "http://127.0.0.1:3456";
}

export function callbackUrl(provider, cfg = loadOAuthConfig(), req = null) {
  const base = req ? publicBaseUrlFromRequest(req, cfg) : cfg?.publicBaseUrl || "http://127.0.0.1:3456";
  return `${base.replace(/\/$/, "")}/api/auth/oauth/callback/${provider}`;
}
...
export function buildAuthorizeUrl(provider, state, cfg = loadOAuthConfig(), req = null) {
  ...
  const redirectUri = callbackUrl(provider, cfg, req);
...
export async function exchangeOAuthCode(provider, code, cfg = loadOAuthConfig(), req = null) {
  if (!isProviderConfigured(provider, cfg)) {
