...
import { attachGeoToDevices } from "./ip-geo.mjs";
...
async function recordVisitSupabase(payload) {
  ...
  if (payload.kind !== "heartbeat") {
    await insertPageHitSupabase(sb, {
...
async function insertPageHitSupabase(sb, row) {
  const { error } = await sb.from("site_page_hits").insert(row);
  if (!error) return;
  const msg = String(error.message || error);
  if (error.code !== "23505" && !/duplicate key|site_page_hits_pkey/i.test(msg)) {
    throw error;
  }
  // Sequence out of sync — insert with explicit next id.
  ...
  const { error: retryErr } = await sb.from("site_page_hits").insert({ ...row, id: nextId });
  if (retryErr) throw retryErr;
  console.warn("[site-visitors] site_page_hits sequence was behind; inserted id", nextId);
}
...
export async function getVisitorHitsByIp(ipRaw, { from = null, to = null, limit = 400 } = {}) {
  ...
  if (!ip || ip.length > 128) return { hits: [], source: "hits" };

  const fromIso = from ? toIsoTs(from) : null;
  const toIsoBound = to ? toIsoTs(to) : null;

  if (isSupabaseBackend()) {
    try {
      const sb = getSupabase();
      let sessQ = sb.from("site_visitor_sessions").select("*").eq("ip", ip).order("last_seen_at", { ascending: false }).limit(100);
      if (fromIso) sessQ = sessQ.gte("last_seen_at", fromIso);
      if (toIsoBound) sessQ = sessQ.lt("last_seen_at", toIsoBound);
      // If range filter yields none but IP has sessions, still load sessions for fallback (unfiltered)
      let { data: sessions, error: sessErr } = await sessQ;
      if (sessErr) throw sessErr;
      if ((!sessions || !sessions.length) && (fromIso || toIsoBound)) {
        const retry = await sb.from("site_visitor_sessions").select("*").eq("ip", ip).order("last_seen_at", { ascending: false }).limit(100);
      ...
      const fetchHits = async (builder) => {
        ...
        if (from) q = q.gte("created_at", fromIso);
        if (to) q = q.lt("created_at", toIsoBound);
        const { data, error } = await q;
        if (error) throw error;
        return data || [];
      };
      ...
      if (merged.length) {
        return { hits: merged.slice(0, limit).map(mapHitRow), source: "hits" };
      }

      // No page_hits in range (sequence was broken) — show session activity instead.
      const sessionHits = (sessions || []).map((s) =>
        mapHitRow({
          id: null,
          visitor_id: s.id,
          path: s.last_path || "/",
          page_title: [s.device_name || s.device_type, s.browser, s.os, `${Number(s.hit_count) || 0} összes jelzés`]
            .filter(Boolean)
            .join(" · "),
          ip: s.ip,
          kind: "session",
      ...
      return { hits: sessionHits.slice(0, limit), source: "sessions" };
    } catch (err) {
      if (isMissingVisitorTableError(err.message)) return { hits: [], source: "hits" };
      throw err;
    }
  }
  ...
  if (rows.length) return { hits: rows.map(mapHitRow), source: "hits" };

  let sessSql = `SELECT * FROM site_visitor_sessions WHERE ip = ?`;
  const sessParams = [ip];
  if (fromSql) {
    sessSql += ` AND last_seen_at >= ?`;
    sessParams.push(fromSql);
  }
  if (toSql) {
    sessSql += ` AND last_seen_at < ?`;
    sessParams.push(toSql);
  }
  sessSql += ` ORDER BY last_seen_at DESC LIMIT ?`;
  sessParams.push(Math.min(limit, 100));
  const sessions = db.prepare(sessSql).all(...sessParams);
  return {
    hits: sessions.map((s) =>
      mapHitRow({
        id: null,
        visitor_id: s.id,
        path: s.last_path || "/",
        page_title: [s.device_name || s.device_type, s.browser, `${Number(s.hit_count) || 0} összes jelzés`]
          .filter(Boolean)
          .join(" · "),
        ip: s.ip,
        kind: "session",
...
async function countSessionsLastSeenSinceSupabase(sinceIso) {
  const sb = getSupabase();
...
export async function getVisitorAdminStats(opts = {}) {
  ...
  if (isSupabaseBackend()) {
    try {
      ...
      let devices = [];
      const fromIso = range.from ? toIsoTs(range.from) : null;
      const toIso = range.to ? toIsoTs(range.to) : null;

      // Prefer last_seen activity (page_hits sequence was broken; sessions still update).
      let q = sb.from("site_visitor_sessions").select("*").order("last_seen_at", { ascending: false }).limit(400);
      if (range.key === "online") {
        q = q.gte("last_seen_at", onlineIso);
      } else if (range.key !== "all" && fromIso) {
        q = q.gte("last_seen_at", fromIso);
        if (toIso) q = q.lt("last_seen_at", toIso);
      }
      const { data, error } = await q;
      if (error) throw error;
      ...
      try {
        if (range.key !== "all" && range.key !== "online" && fromIso) {
          periodCounts = await periodHitCountsSupabase(range.from, range.to);
          period = await countHitsBetweenSupabase(range.from, range.to);
        }
      } catch (hitErr) {
        console.warn("[site-visitors] period hits:", hitErr.message ?? hitErr);
      }

      devices = (data || []).map((row) => {
        const id = String(row.id);
        const fromHits = periodCounts.get(id);
        let periodHitCount;
        if (fromHits != null) periodHitCount = fromHits;
        else if (range.key === "all" || range.key === "online") periodHitCount = Number(row.hit_count) || 0;
        else periodHitCount = 1; // active in range, page_hits missing
        return mapDeviceRow(row, { periodHitCount });
      });

      if (range.key === "all") {
        period = {
          hits: daily.hits || devices.reduce((s, d) => s + (d.periodHitCount || 0), 0),
          unique: Math.max(daily.unique || 0, new Set(devices.map((d) => d.ip || d.id)).size),
        };
      } else if (range.key === "online") {
        period = {
          hits: devices.reduce((s, d) => s + (d.periodHitCount || 0), 0),
          unique: new Set(devices.map((d) => d.ip || d.id)).size,
        };
      } else {
        // If page_hits empty for range, still report session activity.
        const sessionUnique = new Set(devices.map((d) => d.ip || d.id)).size;
        period = {
          hits: period.hits > 0 ? period.hits : devices.length,
          unique: Math.max(period.unique || 0, sessionUnique),
        };
      }

      // Daily/weekly/monthly cards: fall back to sessions last_seen when hits are empty.
      const dailySessions = await countSessionsLastSeenSinceSupabase(new Date(new Date().setHours(0, 0, 0, 0)).toISOString());
      const weeklySessions = await countSessionsLastSeenSinceSupabase(new Date(Date.now() - 7 * 864e5).toISOString());
      const monthlySessions = await countSessionsLastSeenSinceSupabase(new Date(Date.now() - 30 * 864e5).toISOString());
      const dailyOut = {
        hits: daily.hits || 0,
        unique: Math.max(daily.unique || 0, dailySessions),
        sessions: dailySessions,
      };
      const weeklyOut = {
        hits: weekly.hits || 0,
        unique: Math.max(weekly.unique || 0, weeklySessions),
        sessions: weeklySessions,
      };
      const monthlyOut = {
        hits: monthly.hits || 0,
        unique: Math.max(monthly.unique || 0, monthlySessions),
        sessions: monthlySessions,
      };

      devices = await attachGeoToDevices(devices);
  ...
  let period = { hits: 0, unique: 0 };
  let devices = [];
  const fromSql = range.from ? toSqlTs(range.from) : null;
  const toSqlR = range.to ? toSqlTs(range.to) : null;
  ...
  if (range.key === "online") {
    ...
    devices = db
      ...
      .all()
      .map((row) => mapDeviceRow(row, { periodHitCount: Number(row.hit_count) || 0 }));
  } else if (fromSql && toSqlR) {
    devices = db
      .prepare(
        `SELECT * FROM site_visitor_sessions WHERE last_seen_at >= ? AND last_seen_at < ? ORDER BY last_seen_at DESC LIMIT 400`
      )
      .all(fromSql, toSqlR)
      .map((row) => mapDeviceRow(row, { periodHitCount: Number(row.hit_count) || 0 }));
  } else if (fromSql) {
    devices = db
      .prepare(`SELECT * FROM site_visitor_sessions WHERE last_seen_at >= ? ORDER BY last_seen_at DESC LIMIT 400`)
      .all(fromSql)
      .map((row) => mapDeviceRow(row, { periodHitCount: Number(row.hit_count) || 0 }));
  }

  try {
    if (range.key !== "all" && range.key !== "online" && fromSql) {
      ...
      devices = devices.map((d) => ({
        ...d,
        periodHitCount: periodCounts.has(String(d.id)) ? periodCounts.get(String(d.id)) : d.periodHitCount,
      }));
    }
  } catch {
    /* ignore */
  ...
  const sessionUnique = new Set(devices.map((d) => d.ip || d.id)).size;
  if (range.key === "all") {
    period = { hits: daily.hits || devices.reduce((s, d) => s + (d.periodHitCount || 0), 0), unique: Math.max(daily.unique || 0, sessionUnique) };
  } else {
    period = {
      hits: period.hits > 0 ? period.hits : devices.length,
      unique: Math.max(period.unique || 0, sessionUnique),
