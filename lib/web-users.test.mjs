...
test("profil és jelszóváltás", () => {
  ...
  const profile = saveUserProfile(user.id, {
    ...
    postalCode: "1111",
    city: "Budapest",
    street: "Titkos utca 9.",
    accountType: "private",
  });
  assert.equal(profile.firstName, "Anna");
  assert.equal(profile.street, "");
  ...
  assert.equal(again.user.profile.street, "");
...
test("fiók törlés", () => {
  ...
  assert.throws(() => loginUser("torol@local.dev", "abc123abc123"), /regisztrálj/i);
});

test("fiók törlés: hirdetések is törlődnek, belépés nem megy", async () => {
  const { saveListing, getListing, closeDb } = await import("./db.mjs");
  const reg = registerUser("torol-ads@local.dev", "abc123abc123", "abc123abc123");
  const { user, session } = activateUserByToken(reg.activationToken);
  const listing = saveListing(
    {
      hirdetes_cime: "Törlendő teszt autó",
      gyartmany: "Teszt",
      modell: "X",
    },
    null,
  ...
  assert.ok(getListing(listing.id));

  deleteUserAccount(user.id);

  assert.equal(getListing(listing.id), null);
  assert.throws(() => loginUser("torol-ads@local.dev", "abc123abc123"), /regisztrálj/i);
  assert.equal(getUserBySessionToken(session.token), null);
  closeDb();
});

test("elfelejtett jelszó tokennel visszaállítható", () => {
  const reg = registerUser("reset@local.dev", "abc123abc123", "abc123abc123");
  activateUserByToken(reg.activationToken);
  const req = requestPasswordReset("reset@local.dev");
  assert.ok(req.resetToken);
  resetPasswordByToken(req.resetToken, "ujjelszo12345", "ujjelszo12345");
  assert.throws(() => loginUser("reset@local.dev", "abc123abc123"), /Hibás/);
  const login = loginUser("reset@local.dev", "ujjelszo12345");
  assert.equal(login.user.email, "reset@local.dev");
});

test("adminCreateWebUser: azonnal aktív, local@test.bymy.hu", () => {
  const user = adminCreateWebUser({
    email: "mapteszt01",
    displayName: "Map Teszt",
    password: "tesztjelszo12",
    accountType: "private",
  });
  assert.equal(user.email, "mapteszt01@test.bymy.hu");
  assert.equal(user.displayName, "Map Teszt");
  assert.equal(user.emailVerified, true);
  const login = loginUser("mapteszt01", "tesztjelszo12");
  assert.equal(login.user.email, "mapteszt01@test.bymy.hu");
  destroySession(login.session.token);
});
