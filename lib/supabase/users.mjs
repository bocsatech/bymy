...
export async function getUserDetailsForAdmin(userId) {
  async function load(withProfileColumn) {
    const select = withProfileColumn
      ? `id, email, display_name, email_verified, ${withProfileColumn}, created_at, updated_at`
      : `id, email, display_name, email_verified, created_at, updated_at`;
    const { data: row, error } = await sb().from("web_users").select(select).eq("id", userId).maybeSingle();
    if (error) throw error;
    if (!row) throw new Error("Nincs ilyen felhasználó.");
    const profileRaw = withProfileColumn ? row[withProfileColumn] : undefined;
    let profileJson = {};
    try {
      profileJson = profileRaw ? (typeof profileRaw === "string" ? JSON.parse(profileRaw) : profileRaw) : {};
    } catch {
      profileJson = { _parseError: true, raw: profileRaw };
    }
    return {
      id: row.id,
      email: row.email,
  ...
  try {
    return await load("profile_json");
  } catch (error) {
    const msg = String(error?.message || error);
    if (msg.includes("web_users.profile_json") && (msg.includes("does not exist") || msg.includes("nem létezik"))) {
      // Régi / eltérő séma esetén lehet, hogy a mező neve eltér.
      return await load("raw_profile_json");
    }
    throw error;
  }
}
...
export async function updateWebUserForAdmin(userId, patch = {}) {
  ...
  async function doUpdate(profileColumn) {
    const update = { ...baseUpdate, updated_at: new Date().toISOString() };
    update[profileColumn] = JSON.stringify(profileJson ?? {});
  ...
  try {
    return await doUpdate("profile_json");
  } catch (error) {
    const msg = String(error?.message || error);
    if (msg.includes("web_users.profile_json") && (msg.includes("does not exist") || msg.includes("nem létezik"))) {
      return await doUpdate("raw_profile_json");
    }
    throw error;
  }
}
...
function companyFieldsFrom(profile, existing = {}) {
  return {
    ...
    companyAddress: strProfileField(profile, existing, "companyAddress"),
...
export async function saveUserProfile(userId, profile) {
  const { data: row } = await sb().from("web_users").select("email, profile_json").eq("id", userId).maybeSingle();
  if (!row?.email) throw new Error("A profil mentése sikertelen (nincs ilyen felhasználó).");
  const existing = profileFromRow(row);
  const next = {
    salutation: String(profile.salutation ?? existing.salutation ?? "").trim(),
    ...
    accountType: normalizeAccountType(existing.accountType),
  };
