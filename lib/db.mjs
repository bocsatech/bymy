...
import { enrichListingsWithOwnerProfileLocationSync } from "./listing-preview-location.mjs";
import { applyImporterProfileToListingForm } from "./listing-address-from-profile.mjs";
import { getUserById } from "./web-users.mjs";
...
export function listListingsWithPreview({
  ...
  const fetchLimit = needFilter
    ? Math.min(Math.max((off + max) * 40, 400), 5000)
    : off + max;
  ...
  let items = rows.map((row) => {
    ...
    const seed = preview.imageUrls?.length ? preview.imageUrls : foKep ? [foKep] : [];
    const urls = seed
      .map((url) => {
        try {
          return displayImageUrl(upgradeHaImageUrl(url) || url);
        } catch {
          return "";
        }
      })
    ...
    preview.imageUrls = urls.slice(0, 3);
    ...
    return {
      ...row,
      fo_kep: urls[0] || (/^https?:\/\//i.test(foKep) ? foKep : ""),
      ...stats,
      hirdetes_cime,
      preview,
      form,
  ...
  if (needFilter) {
    items = items.filter((item) => resolveListingVertical(item) === want);
  }
  items = items.slice(off, off + max);
  return enrichListingsWithOwnerProfileLocationSync(items, getUserById);
}
...
export function countListingsPublic({ status = null, vertical = null } = {}) {
  const counts = countNavListings({ status: status || "feladott" });
...
export function patchListingFormFields(listingId, fields = {}) {
  const id = Number(listingId);
  if (!Number.isFinite(id) || id <= 0) return null;
  if (!getListing(id)) return null;
  const db = getDb();
  const del = db.prepare(`DELETE FROM listing_cells WHERE listing_id = ? AND field_key = ?`);
...
export function listListingsByOwner({
  ...
  let ownerRows = db
    ...
    .all(String(uid), max + 40);

  if (!ownerRows.length) {
    ownerRows = db
      .prepare(
        `SELECT listing_id, value FROM listing_cells
         WHERE field_key = 'owner_user_id'
         LIMIT 5000`
      )
      .all()
      .filter((row) => Number(String(row.value ?? "").trim()) === uid);
  }
  ...
  const cellsById = loadCellsByListingIds(rows.map((row) => row.id));
  const keySet = Array.isArray(cellKeys) && cellKeys.length ? new Set(cellKeys) : null;
  return rows.map((row) => {
    let cells = (cellsById.get(row.id) ?? []).map((cell) => sanitizeListingCell(cell));
    if (keySet) cells = cells.filter((cell) => keySet.has(cell.field_key));
    const hirdetes_cime =
      sanitizeListingPlainText(row.hirdetes_cime) || `Hirdetés #${row.id}`;
    const preview = buildPreviewFromCells(cells, { ...row, hirdetes_cime });
    const rawFo = String(row.fo_kep ?? "").trim();
...
export function getListingOwnerMeta(listingId) {
  const id = Number(listingId);
  if (!Number.isFinite(id) || id <= 0) return null;
  const db = getDb();
  const listing = db.prepare("SELECT id, status FROM listings WHERE id = ?").get(id);
  if (!listing) return null;
  const cell = db
    .prepare(
...
function upsertListingMeta(db, id, formData, status, { replaceImport = false } = {}) {
...
function replaceCells(db, listingId, cells, { replaceImport = false } = {}) {
  const existing = db
...
function maybeFillOwnerLocationFromProfile(clean, userId) {
  const uid = Number(userId);
...
export function saveListing(
  ...
  clean = maybeFillOwnerLocationFromProfile(clean, userId);
  ...
  if (listingId) {
    ...
    upsertListingMeta(db, listingId, clean, listingStatus, { replaceImport });
    replaceCells(db, listingId, cells, { replaceImport });
    return getListing(listingId);
  }
...
export async function backfillListingFeed() {
  return { ok: false, error: "listing_feed csak Supabase módban", upserted: 0 };
}
