...
test("isListingImageMissing / displayImageUrl: https → proxy", async () => {
  ...
  assert.equal(
    mod.isAllowedRemoteImageUrl("https://bymy.hu/storage/v1/object/public/listing-images/x.webp"),
    true
  );
  assert.equal(mod.displayImageUrl("https://evil.example/x.jpg"), "");
  assert.equal(mod.isListingImageMissing(""), true);
  const thumb = "https://img.hasznaltautocdn.com/118x88/23113337/26375069.jpg";
  assert.equal(
    mod.displayImageUrl(thumb),
    "https://img.hasznaltautocdn.com/2048x1536/23113337/26375069.jpg"
  );
  rmSync(dir, { recursive: true, force: true });
  delete process.env.AUTOSWEB_UPLOADS_PATH;
});
...
test("upgradeHaImageUrl: 118x88 → 2048x1536", async () => {
  const mod = await import(`./listing-image.mjs?t=${Date.now() + 77}`);
  const thumb = "https://img.hasznaltautocdn.com/118x88/23113337/26375069.jpg";
  const hq = "https://img.hasznaltautocdn.com/2048x1536/23113337/26375069.jpg";
  const list = "https://img.hasznaltautocdn.com/640x480/23113337/26375069.jpg";
  assert.equal(mod.upgradeHaImageUrl(thumb), hq);
  assert.equal(mod.upgradeHaImageUrl(hq), hq);
  assert.equal(
    mod.upgradeHaImageUrl("https://img.hasznaltautocdn.com/800x600/23113337/26375069.jpg?x=1"),
    hq
  );
  assert.equal(mod.listHaImageUrl(thumb), list);
  assert.equal(mod.listHaImageUrl(hq), list);
  assert.equal(mod.isHaThumbImageUrl(thumb), true);
  assert.equal(mod.isHaThumbImageUrl(hq), false);
  assert.equal(mod.displayImageUrl(thumb), hq);
  const supabase =
    "https://abc.supabase.co/storage/v1/object/public/listings/1/a.jpg";
  assert.equal(mod.displayImageUrl(supabase), supabase);
  const candidates = mod.haImageUrlCandidates(thumb);
  assert.equal(candidates[0], hq);
  assert.ok(candidates.includes("https://img.hasznaltautocdn.com/1024x768/23113337/26375069.jpg"));
  assert.ok(candidates.includes(thumb));
});
