...
import { enrichListingsWithOwnerProfileLocation } from "../listing-preview-location.mjs";
...
async function fetchListingRows({ limit = 50, status = null, vertical = null } = {}) {
  ...
  if (error) {
    if (useVertical && /vertical/i.test(String(error.message || ""))) {
      ...
      return fetchListingRows({ limit, status, vertical: null });
    }
    throw error;
  }
  return data ?? [];
...
/** Menü számlálók — csak vertical cellák, teljes preview nélkül. */
let navCountsCache = { at: 0, value: null, ttlMs: 60_000 };

function feedCacheKey({ limit, status, vertical }) {
  return `${normalizeListingStatus(status || "")}|${String(vertical || "").trim().toLowerCase()}|${Number(limit) || 50}`;
}
...
async function listListingsWithPreviewUncached({ limit = 50, status = null, vertical = null } = {}) {
  const fromFeed = await listListingFeed(sb(), { limit, status, vertical });
  if (fromFeed) {
    if (fromFeed.length === 0) {
      ...
      return enrichListingsWithOwnerProfileLocation(fromFeed, getUserById);
    }
  }
  ...
  let selectedRows;
  if (hasVerticalCol) {
    selectedRows = await fetchListingRows({ limit, status, vertical: want });
  } else if (needFilter) {
    // Régi út: sok ID + könnyű vertical cellák, teljes EAV csak a végső limithoz.
    const fetchLimit = Math.min(Math.max(limit * 40, 400), 2000);
    ...
    selectedRows = await fetchListingRows({ limit, status });
  }
  ...
  let items = selectedRows.map((row) => {
    const cells = (cellsById.get(Number(row.id)) ?? []).map((cell) => sanitizeListingCell(cell));
    return listingRowWithPreview(row, cells);
  });
  items = await enrichListingsWithOwnerProfileLocation(items, getUserById);
  return items;
}

export async function listListingsWithPreview({ limit = 50, status = null, vertical = null } = {}) {
  const key = feedCacheKey({ limit, status, vertical });
  const now = Date.now();
  ...
  const task = listListingsWithPreviewUncached({ limit, status, vertical })
    .then((value) => {
...
export async function countNavListings({ status = "feladott" } = {}) {
  ...
  if (!ids.length) {
    // Üres eredményt ne tartsuk 60 mp-ig (tranziens / hideg start).
    navCountsCache = { at: now, value: counts, ttlMs: 5_000 };
    return counts;
  }
  ...
  // Párhuzamos csomagok — ne 60+ sorozatos round-trip.
  const CONCURRENCY = 8;
...
export async function listListingsByOwner({
  ...
  const { data: ownerCells, error } = await sb()
    .from("listing_cells")
    .select("listing_id")
    .eq("field_key", "owner_user_id")
    .eq("value", String(uid))
    .limit(Math.min(max + 20, 520));
  if (error) throw error;
  ...
  let ids = [
    ...new Set(
      (ownerCells ?? [])
        .map((cell) => Number(cell.listing_id))
