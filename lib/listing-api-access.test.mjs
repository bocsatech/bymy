...
test("sanitizeListingList: feed vékonyítás", () => {
  const out = sanitizeListingList([
    {
      id: 2,
      user_id: 9,
      fo_kep: "https://img.hasznaltautocdn.com/2048x1536/23113337/26375069.jpg",
      preview: {
        imageUrls: [
      ...
      form: {
        telefon: "titok",
        leiras: "y".repeat(500),
        felszereltseg: Array.from({ length: 80 }, (_, i) => `f${i}`),
        gyartmany: "BMW",
      },
    },
  ]);
  assert.equal(out[0].user_id, undefined);
  assert.deepEqual(out[0].preview.imageUrls, [
    "https://img.hasznaltautocdn.com/640x480/23113337/26375069.jpg",
  ]);
  assert.equal(out[0].fo_kep, "https://img.hasznaltautocdn.com/640x480/23113337/26375069.jpg");
  assert.ok(out[0].preview.leiras.length <= 181);
  assert.equal(out[0].form.telefon, undefined);
  assert.equal(out[0].form.gyartmany, "BMW");
  assert.equal(out[0].form.felszereltseg.length, 24);
  assert.ok(out[0].form.leiras.length <= 221);
});
