...
import { buildListingDetailView, maskPhone, formatBrandDisplayName } from "./listing-detail-view.mjs";
...
test("márkanév megjelenítés: csupa nagybetű → címkeforma", () => {
  assert.equal(formatBrandDisplayName("ALFA ROMEO"), "Alfa Romeo");
  assert.equal(formatBrandDisplayName("FORD"), "Ford");
  assert.equal(formatBrandDisplayName("Mercedes-Benz"), "Mercedes-Benz");
});
...
test("hirdetésnézet a feltöltött autóadatokból, magyar címkékkel", () => {
  ...
  assert.equal(view.title, "Ford Kuga (2022)");
  ...
  assert.ok(view.bodyTech.some((row) => row.label === "Jármű típusa" && /crossover|terepjáró/i.test(row.value)));
  ...
  assert.equal(view.categoryLabel, "Autók");
  assert.equal(view.brand, "Ford");
