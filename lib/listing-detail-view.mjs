...
function equipmentList(form) {
  ...
  if (klima && !/^nincs$/i.test(klima)) items.push(klima);
...
/** Willhaben-szerű: „Felhasználó ezóta: 03/2014” */
...
export function buildListingDetailView(listing = {}) {
  ...
  const vertical = str(form, "hirdetes_vertical").toLowerCase();
  const categoryHref = vertical === "teher" ? "/teherauto.html" : "/auto.html";
  const categoryLabel = vertical === "teher" ? "Teherautó" : "Autó";
  ...
  /** Kompatibilitás (adásvételi / régi kliensek). */
  const basics = [
    ...vehicleSpecs.filter((row) =>
      ["Sebességváltó", "Futásteljesítmény", "Hajtás"].includes(row.label)
    ),
    ...motorSpecs.filter((row) => ["Teljesítmény", "Üzemanyag"].includes(row.label)),
  ];
  const bodyTech = [
    ...vehicleSpecs.filter((row) =>
      ["Állapot", "Jármű típusa", "Ajtók száma", "Ülések száma", "Külső szín"].includes(row.label)
    ),
