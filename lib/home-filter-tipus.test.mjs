...
test("dízel szűrő: uzemanyagQuick dobkerék érték (Dízel) és Diesel hirdetés", () => {
  const items = [
    listing({ uzemanyag: "Diesel" }, { id: "d1" }),
    listing({ uzemanyag: "Benzin" }, { id: "b1" }),
  ];
  const filters = { ...emptyFilters(), uzemanyagQuick: "Dízel" };
  const result = filterListingsBySidebar(items, filters);
  assert.equal(result.length, 1);
  assert.equal(result[0].id, "d1");
});

test("dízel szűrő: uzemanyag mező diesel id és Dízel hirdetés", () => {
  const items = [
    listing({ uzemanyag: "Dízel" }, { id: "d1" }),
    listing({ uzemanyag: "Benzin" }, { id: "b1" }),
  ];
  const filters = { ...emptyFilters(), uzemanyag: "diesel" };
  const result = filterListingsBySidebar(items, filters);
  assert.equal(result.length, 1);
  assert.equal(result[0].id, "d1");
});

test("dízel szűrő: uzemanyag mező és Dízel hirdetés", () => {
  const items = [
    listing({ uzemanyag: "Dízel" }, { id: "d1" }),
    listing({ uzemanyag: "Benzin" }, { id: "b1" }),
  ];
  const filters = { ...emptyFilters(), uzemanyag: "Dízel" };
  const result = filterListingsBySidebar(items, filters);
  assert.equal(result.length, 1);
  assert.equal(result[0].id, "d1");
});

test("dízel szűrő: asztali picker uzemanyagok tömb", () => {
  const items = [
    listing({ uzemanyag: "Dízel" }, { id: "d1" }),
    listing({ uzemanyag: "Diesel" }, { id: "d2" }),
    listing({ uzemanyag: "Benzin" }, { id: "b1" }),
  ];
  const filters = { ...emptyFilters(), uzemanyagok: ["Dízel"] };
  const result = filterListingsBySidebar(items, filters);
