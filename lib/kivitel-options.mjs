...
/** Régi / import értékek → új lista. */
const ALIASES = {
  szedan: "Sedan",
  sedan: "Sedan",
  "lepcoshátu": "Lépcsőshátú",
  ...
  "varosi terepjáró (crossover)": "Városi terepjáró (crossover)",
  terepjáró: "Terepjáró",
  terepjaro: "Terepjáró",
};
...
export function kivitelOptionsHtml({ emptyLabel = "Válasszon", emptyValue = "" } = {}) {
  const lines = [`<option value="${emptyValue}">${emptyLabel}</option>`];
  for (const opt of KIVITEL_OPTIONS) {
