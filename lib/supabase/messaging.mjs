...
async function publicConversation(conv, userId, imageMap = null) {
...
async function conversationIdsWithMessages(conversationIds) {
  const ids = [...new Set(conversationIds.map((id) => Number(id)).filter((id) => id > 0))];
  ...
  if (error) throw error;
  return new Set((data ?? []).map((row) => Number(row.conversation_id)));
}

async function purgeEmptyConversations(rows) {
  if (!rows?.length) return [];
  const withMessages = await conversationIdsWithMessages(rows.map((row) => row.id));
  const emptyIds = rows.filter((row) => !withMessages.has(Number(row.id))).map((row) => row.id);
  if (emptyIds.length) {
    await sb().from("conversations").delete().in("id", emptyIds);
  }
  return rows.filter((row) => withMessages.has(Number(row.id)));
}
...
export async function listConversations(userId) {
  ...
  const { data, error } = await sb()
    ...
    .order("id", { ascending: false });
  if (error) throw error;
  const rows = await purgeEmptyConversations(data ?? []);
  const imageMap = await listingImageUrlsByIds(rows.map((row) => row.listing_id));
  return Promise.all(rows.map((c) => publicConversation(c, userId, imageMap)));
}

async function resolveSellerFromListing(listingId) {
  ...
  const { data } = await sb()
    ...
    .maybeSingle();
  const owner = Number(data?.value);
  return Number.isFinite(owner) && owner > 0 ? owner : 0;
}

async function resolveConversationParties(userId, input) {
  await initMessagingSchema();
  ...
  if (!Number.isFinite(sellerId) || sellerId <= 0) {
    sellerId = await resolveSellerFromListing(listingId);
  }
  if (!sellerId) {
...
async function findExistingConversationRow(listingId, buyerId, sellerId) {
  const { data } = await sb()
    .from("conversations")
    .select("*")
...
async function purgeEmptyConversationRow(conv) {
  if (!conv) return true;
  const withMessages = await conversationIdsWithMessages([conv.id]);
  if (withMessages.has(Number(conv.id))) return false;
  await sb().from("conversations").delete().eq("id", conv.id);
  return true;
}

async function insertConversationRow(userId, parties) {
  const now = new Date().toISOString();
  const { data: inserted, error } = await sb()
    .from("conversations")
    .insert({
      listing_id: parties.listingId,
      listing_title: parties.title,
      listing_price_label: parties.priceLabel,
      listing_code: parties.code,
      listing_meta: parties.meta,
      buyer_id: userId,
      seller_id: parties.sellerId,
...
export async function findConversationForListing(userId, input) {
  const parties = await resolveConversationParties(userId, input);
  const existing = await findExistingConversationRow(parties.listingId, userId, parties.sellerId);
  if (!existing) return null;
  if (await purgeEmptyConversationRow(existing)) return null;
  return publicConversation(existing, userId);
}
...
export async function startConversationWithInitial(userId, input) {
  ...
  let row = await findExistingConversationRow(parties.listingId, userId, parties.sellerId);
  if (row && (await purgeEmptyConversationRow(row))) row = null;
  if (!row) row = await insertConversationRow(userId, parties);

  await sendMessage(userId, row.id, { body: text, attachment });
  const { data: fresh } = await sb().from("conversations").select("*").eq("id", row.id).maybeSingle();
  return publicConversation(fresh ?? row, userId);
}
