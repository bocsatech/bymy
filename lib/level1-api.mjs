...
import { getVisitorAdminStats } from "./site-visitors.mjs";
...
async function deliverLevel1Otp(pending) {
  ...
  if (isSmtpConfigured()) {
    ...
    return { ok: true, otpSent: true, username: pending.username, emailMasked };
  }
  if (!allowDevCode) {
    const err = new Error("Email küldés nincs beállítva. Vercel env: SMTP_USER, SMTP_PASS.");
...
export async function handleLevel1Api(req, res, pathname) {
  try {
    ...
    if (pathname === "/api/level1/ingatlan-wheel-schema" && req.method === "GET") {
      sendJson(res, 200, { schema: await getIngatlanWheelSchema() });
      return;
    }
    ...
    if (pathname === "/api/level1/ingatlan-wheel-schema" && req.method === "PUT") {
      const body = await readBody(req);
      const schema = await saveIngatlanWheelSchema(body.schema || body);
    ...
    if (pathname === "/api/level1/visitors" && req.method === "GET") {
      sendJson(res, 200, await getVisitorAdminStats());
      return;
    }
