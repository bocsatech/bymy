...
async function loadAdminCellsByListingIds(ids) {
  if (!ids.length) return new Map();
  const { data, error } = await sb()
    .from("listing_cells")
    .select("listing_id, field_key, value")
    .in("listing_id", ids)
    .in("field_key", ADMIN_LISTING_CELL_KEYS);
  if (error) throw error;
  const map = new Map();
  for (const row of data ?? []) {
    if (!map.has(row.listing_id)) map.set(row.listing_id, []);
...
export async function listListingsForAdmin({ limit = 150, vertical = null, excludeVertical = null } = {}) {
  const needFilter = Boolean(vertical || excludeVertical);
  const fetchLimit = needFilter ? Math.min(Math.max(limit * 6, 300), 600) : limit;
  const rows = await listListings({ limit: fetchLimit });
  const cellsById = await loadAdminCellsByListingIds(rows.map((row) => row.id));
...
export async function listListingsWithPreview({ limit = 50, status = null, vertical = null } = {}) {
  ...
  const rows = await listListings({ limit: fetchLimit, status });
  const cellsById = await loadCellsByListingIds(rows.map((r) => r.id));
