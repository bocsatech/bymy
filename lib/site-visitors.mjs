...
import { ensureSupabaseVisitorSchema, visitorSchemaHint } from "./supabase/visitor-schema.mjs";
...
export async function recordPageVisit(req, body = {}) {
  ...
  if (isSupabaseBackend()) {
    try {
      await ensureSupabaseVisitorSchema();
...
export async function getVisitorAdminStats() {
  ...
  if (isSupabaseBackend()) {
    try {
      const boot = await ensureSupabaseVisitorSchema();
      if (!boot.ok && !boot.cached) {
        throw new Error(boot.error || visitorSchemaHint());
      }
      const sb = getSupabase();
      ...
      return {
        ...
        devices: [],
        onlineWindowMinutes: 5,
        warning: msg.includes("site_page_hits") || msg.includes("site_visitor_sessions") ? hint : msg,
        schemaMissing: true,
      };
    }
  }
