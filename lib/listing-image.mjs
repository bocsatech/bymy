...
import { resolveFilesystemImageMediaFile, IMAGE_MEDIA_URL_PREFIX } from "./filesystem-image-storage.mjs";
...
export function isAllowedRemoteImageUrl(raw) {
  try {
    ...
    return (
      ...
      host === "bymy.hu" ||
      host.endsWith(".bymy.hu") ||
      host === "img.bymy.hu" ||
      host === "bymy.vercel.app" ||
...
/** Lista / mozaik kártya — ne a teljes HQ-t töltsük a feedben. */
export const HA_IMAGE_LIST_SIZE = "640x480";
...
function rewriteHaCdnSize(src, size) {
  let url = String(src || "").trim();
  if (url.startsWith("//")) url = `https:${url}`;
  if (!/^https?:\/\//i.test(url)) return "";
  try {
    const u = new URL(url);
    if (!isHaCdnHost(u.hostname)) return "";
...
export function upgradeHaImageUrl(src) {
  ...
  try {
    ...
    if (isHaCdnHost(u.hostname)) {
      const hq = rewriteHaCdnSize(url, HA_IMAGE_HQ_SIZE);
      if (hq) return hq;
...
/**
 * Lista-kártya CDN méret (640x480). HQ / egyéb méret → listás; nem-HA URL változatlan.
 * Mentéshez továbbra is az upgradeHaImageUrl (HQ) kell.
 */
export function listHaImageUrl(src) {
  const raw = String(src || "").trim();
  if (!raw) return "";
  const list = rewriteHaCdnSize(raw, HA_IMAGE_LIST_SIZE);
  if (list) return list;
...
export function displayImageUrl(foKep) {
  let path = String(foKep || "").trim();
  if (!path) return "";
  // Ha már proxy URL, bontsuk ki a belső címet
  if (path.startsWith("/api/media/proxy")) {
    try {
      const inner = new URL(path, "https://bymy.local").searchParams.get("url");
      if (inner) path = inner;
    } catch {
    }
  }
  if (/^https?:\/\//i.test(path)) {
    ...
    try {
      const host = new URL(path).hostname.replace(/^www\./, "").toLowerCase();
      // Nyilvános CDN / storage: közvetlen URL (Vercel edge proxy auth/502 nélkül)
      if (
        host === "hasznaltautocdn.com" ||
        host.endsWith(".hasznaltautocdn.com") ||
        host.endsWith(".supabase.co") ||
  ...
  if (path.startsWith(`${IMAGE_MEDIA_URL_PREFIX}/`)) {
    return resolveFilesystemImageMediaFile(path) ? path : "";
  }
...
export function isListingImageMissing(foKep) {
  ...
  if (path.startsWith(`${IMAGE_MEDIA_URL_PREFIX}/`)) {
    return !resolveFilesystemImageMediaFile(path);
  }
