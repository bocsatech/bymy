...
test("parseListingHtml: teljes táblázat → form mezők", () => {
  ...
  assert.equal(form.kivitel, "Városi terepjáró (crossover)");
...
test("parseListingHtml kihagyja a Hiba! h1-et", () => {
  ...
  assert.match(parsed.cim, /Audi A6/i);
  assert.doesNotMatch(parsed.cim, /hiba/i);
  assert.equal(parsed.nyersAdatok["Márka"], "Audi");
});
...
test("splitChainedHaValue levágja a következő címkét", async () => {
  const { splitChainedHaValue, parseBodyTextAttributes, mergeAttributeMaps } = await import("./parse-listing.mjs");
  assert.equal(
    splitChainedHaValue("Fekete (metál) Saját tömeg: 1 700 kg Össztömeg: 2 215 kg"),
    "Fekete (metál)"
  );
  assert.equal(splitChainedHaValue("Szürke Saját tömeg: 1 740 kg Össztömeg:"), "Szürke");
  const fromBody = parseBodyTextAttributes(
    "Szín: Fekete (metál) Saját tömeg: 1 700 kg Össztömeg: 2 215 kg\nSaját tömeg: 1 700 kg\nÖssztömeg: 2 215 kg"
  );
  assert.equal(fromBody["Szín"], "Fekete (metál)");
  assert.match(fromBody["Saját tömeg"] || "", /1\s*700/);
  const merged = mergeAttributeMaps(
    { Szín: "Fekete (metál)" },
    { Szín: "Fekete (metál) Saját tömeg: 1 700 kg Össztömeg: 2 215 kg" }
  );
  assert.equal(merged["Szín"], "Fekete (metál)");
});
