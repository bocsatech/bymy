import { sendMail, isSmtpConfigured } from "./mail.mjs";
import { readJsonBody } from "./read-json-body.mjs";
import { rateLimit, clientIp } from "./rate-limit.mjs";
import {
  startLevel1Login,
  resendLevel1Otp,
  verifyLevel1Otp,
  getLevel1TokenFromRequest,
  getLevel1AdminBySession,
  destroyLevel1Session,
  level1CookieHeader,
  clearLevel1CookieHeader,
  getFormLayout,
  saveFormLayout,
  getIngatlanWheelSchema,
  saveIngatlanWheelSchema,
  getSearchCylinderMenu,
  saveSearchCylinderMenu,
  getSearchCylinderPublic,
  getKivitelMenu,
  saveKivitelMenu,
  getKivitelMenuPublic,
  getAkkuSearchMenu,
  saveAkkuSearchMenu,
  getAkkuSearchMenuPublic,
  readAkkuSearchMenuRaw,
  getIngatlanTipusFieldsConfig,
  saveIngatlanTipusFieldsConfig,
  getIngatlanTipusFieldsPublic,
  listLayoutCategories,
  normalizeLayoutCategory,
  level1UnlockSql,
  maskEmail,
} from "./level1.mjs";
import { SEARCH_CYLINDER_IMAGE_PRESETS } from "./search-cylinder-menu.mjs";
import { publicAkkuSearchSection } from "./akku-search-menu.mjs";
import { listBlockedIps, blockIp, unblockIp } from "./site-ip-blocks.mjs";
import {
  listBannedUsers,
  banUserRecord,
  unbanEmail,
} from "./banned-users.mjs";
import { mergeAdminFlagsPatch } from "./user-admin-flags.mjs";
import {
  listWebUsersForAdmin,
  deleteUserAccount,
  getUserDetailsForAdmin,
  mergeUserProfileJson,
  updateWebUserForAdmin,
  adminCreateWebUser,
} from "./web-users-store.mjs";
import { listListingsWithPreview, updateListingStatus, deleteListing, saveListing } from "./db-store.mjs";
import { resolveListingVertical } from "./listing-vertical.mjs";
import {
  getHubPromoAdmin,
  uploadHubPromoImage,
  updateHubPromoImage,
  deleteHubPromoImage,
} from "./hub-promo.mjs";
import {
  getAdFormDeskGuideAdmin,
  uploadAdFormDeskGuideImage,
  updateAdFormDeskGuideImage,
  deleteAdFormDeskGuideImage,
} from "./ad-form-desk-guide.mjs";
import { getVisitorAdminStats, getVisitorPageHits, getVisitorHitsByIp, resolveVisitorTimeRange } from "./site-visitors.mjs";
import { ensureSupabaseVisitorSchema } from "./supabase/visitor-schema.mjs";
import { listPartnerProfilesForAdmin, reviewPartnerProfile } from "./partner-profiles.mjs";
import {
  ADMIN_BACKUP_CATEGORIES,
  createAdminBackup,
  listAdminBackups,
  previewAdminBackup,
  restoreAdminBackup,
} from "./level1-backup.mjs";

function sendJson(res, status, data, headers = {}) {
  res.writeHead(status, {
    "Content-Type": "application/json; charset=utf-8",
    "Cache-Control": "no-store",
    ...headers,
  });
  res.end(JSON.stringify(data));
}

async function requireAdmin(req, res) {
  const admin = await getLevel1AdminBySession(getLevel1TokenFromRequest(req));
  if (!admin) {
    sendJson(res, 401, { error: "Admin belépés szükséges." });
    return null;
  }
  return admin;
}

async function deliverLevel1Otp(pending) {
  const emailMasked = pending.emailMasked || maskEmail(pending.email);
  const base = { ok: true, username: pending.username, emailMasked };
  const maybeDevCode =
    String(process.env.ALLOW_DEV_SECRETS ?? "").trim() === "1" ? { devCode: pending.code } : {};
  if (isSmtpConfigured()) {
    try {
      await Promise.race([
        sendMail({
          to: pending.email,
          subject: "Bocsatech belépési kód",
          text: `A belépési kódod: ${pending.code}\n10 percig érvényes.\nHa nem te kérted, hagyd figyelmen kívül.`,
          html: `<p>A belépési kódod: <strong>${pending.code}</strong></p><p>10 percig érvényes.</p>`,
        }),
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error("SMTP válasz túl lassú (12 s).")), 12_000)
        ),
      ]);
      return { ...base, otpSent: true };
    } catch (mailErr) {
      return {
        ...base,
        otpSent: false,
        smtpWarning: String(mailErr.message ?? mailErr),
        ...maybeDevCode,
      };
    }
  }
  return {
    ...base,
    otpSent: false,
    smtpRequired: true,
    ...maybeDevCode,
  };
}

function assertLevel1Rate(req, res, bucket, { limit = 12, windowMs = 15 * 60 * 1000 } = {}) {
  const ip = clientIp(req);
  const result = rateLimit(`${bucket}:${ip}`, { limit, windowMs });
  if (!result.ok) {
    sendJson(
      res,
      429,
      { error: "Túl sok kísérlet. Próbáld újra később." },
      { "Retry-After": String(result.retryAfterSec || 60) }
    );
    return false;
  }
  return true;
}

export async function handleLevel1Api(req, res, pathname) {
  try {
    if (pathname === "/api/level1/me" && req.method === "GET") {
      const admin = await getLevel1AdminBySession(getLevel1TokenFromRequest(req));
      sendJson(res, 200, { admin });
      return;
    }

    if (pathname === "/api/level1/login" && req.method === "POST") {
      if (!assertLevel1Rate(req, res, "level1-login", { limit: 10, windowMs: 15 * 60 * 1000 })) return;
      const body = await readJsonBody(req);
      const pending = await startLevel1Login(body.username, body.password);
      if (pending.skipOtp && pending.session && pending.admin) {
        sendJson(
          res,
          200,
          { ok: true, skipOtp: true, admin: pending.admin, username: pending.username },
          { "Set-Cookie": level1CookieHeader(pending.session.token, pending.session.expires) }
        );
        return;
      }
      sendJson(res, 200, await deliverLevel1Otp(pending));
      return;
    }

    if (pathname === "/api/level1/resend-otp" && req.method === "POST") {
      if (!assertLevel1Rate(req, res, "level1-resend-otp", { limit: 8, windowMs: 15 * 60 * 1000 })) return;
      const body = await readJsonBody(req);
      const pending = await resendLevel1Otp(body.username);
      sendJson(res, 200, await deliverLevel1Otp(pending));
      return;
    }

    if (pathname === "/api/level1/otp" && req.method === "POST") {
      if (!assertLevel1Rate(req, res, "level1-otp", { limit: 12, windowMs: 15 * 60 * 1000 })) return;
      const body = await readJsonBody(req);
      const { admin, session } = await verifyLevel1Otp(body.username, body.code);
      sendJson(
        res,
        200,
        { ok: true, admin },
        { "Set-Cookie": level1CookieHeader(session.token, session.expires) }
      );
      return;
    }

    if (pathname === "/api/level1/logout" && req.method === "POST") {
      await destroyLevel1Session(getLevel1TokenFromRequest(req));
      sendJson(res, 200, { ok: true }, { "Set-Cookie": clearLevel1CookieHeader() });
      return;
    }

    if (pathname === "/api/level1/form-layout" && req.method === "GET") {
      const url = new URL(req.url || "/", "http://localhost");
      const category = normalizeLayoutCategory(url.searchParams.get("category") || "szemelyauto");
      sendJson(res, 200, {
        layout: await getFormLayout(category),
        category,
        categories: listLayoutCategories(),
      });
      return;
    }

    if (pathname === "/api/level1/ingatlan-wheel-schema" && req.method === "GET") {
      const url = new URL(req.url || "/", "http://localhost");
      const variant = url.searchParams.get("variant") || "ingatlan";
      sendJson(res, 200, {
        schema: await getIngatlanWheelSchema(variant),
        variant,
      });
      return;
    }

    if (pathname === "/api/level1/search-cylinder" && req.method === "GET") {
      const data = await getSearchCylinderPublic();
      sendJson(res, 200, data);
      return;
    }

    if (pathname === "/api/level1/kivitel-menu" && req.method === "GET") {
      sendJson(res, 200, await getKivitelMenuPublic());
      return;
    }

    if (pathname === "/api/level1/akku-search-menu" && req.method === "GET") {
      sendJson(res, 200, await getAkkuSearchMenuPublic());
      return;
    }

    if (pathname === "/api/level1/ingatlan-tipus-fields" && req.method === "GET") {
      sendJson(res, 200, await getIngatlanTipusFieldsPublic());
      return;
    }

    const admin = await requireAdmin(req, res);
    if (!admin) return;

    if (pathname === "/api/level1/hub-promo" && req.method === "GET") {
      sendJson(res, 200, await getHubPromoAdmin());
      return;
    }

    if (pathname === "/api/level1/hub-promo/upload" && req.method === "POST") {
      const body = await readJsonBody(req);
      sendJson(
        res,
        200,
        await uploadHubPromoImage(body.image, { href: body.href, alt: body.alt })
      );
      return;
    }

    if (pathname === "/api/level1/hub-promo/image" && req.method === "PATCH") {
      const body = await readJsonBody(req);
      sendJson(
        res,
        200,
        await updateHubPromoImage(body.id || body.imageId, { href: body.href, alt: body.alt })
      );
      return;
    }

    if (pathname === "/api/level1/hub-promo/image" && req.method === "DELETE") {
      const body = await readJsonBody(req);
      sendJson(res, 200, await deleteHubPromoImage(body.id || body.imageId));
      return;
    }

    if (pathname === "/api/level1/ad-form-desk-guide" && req.method === "GET") {
      sendJson(res, 200, await getAdFormDeskGuideAdmin());
      return;
    }

    if (pathname === "/api/level1/ad-form-desk-guide/upload" && req.method === "POST") {
      const body = await readJsonBody(req);
      sendJson(
        res,
        200,
        await uploadAdFormDeskGuideImage(body.slot, body.image, { alt: body.alt })
      );
      return;
    }

    if (pathname === "/api/level1/ad-form-desk-guide/image" && req.method === "PATCH") {
      const body = await readJsonBody(req);
      sendJson(res, 200, await updateAdFormDeskGuideImage(body.slot, { alt: body.alt }));
      return;
    }

    if (pathname === "/api/level1/ad-form-desk-guide/image" && req.method === "DELETE") {
      const body = await readJsonBody(req);
      sendJson(res, 200, await deleteAdFormDeskGuideImage(body.slot));
      return;
    }

    if (pathname === "/api/level1/unlock-sql" && req.method === "GET") {
      sendJson(res, 200, { sql: level1UnlockSql(admin.username) });
      return;
    }

    if (pathname === "/api/level1/form-layout" && req.method === "PUT") {
      const body = await readJsonBody(req);
      const category = normalizeLayoutCategory(body.category || "szemelyauto");
      try {
        const layout = await saveFormLayout(body.layout || body, category);
        sendJson(res, 200, { ok: true, layout, category });
      } catch (error) {
        if (error?.code === "LAYOUT_MASTER_ONLY") {
          sendJson(res, 400, { error: error.message, code: error.code });
          return;
        }
        throw error;
      }
      return;
    }

    if (pathname === "/api/level1/ingatlan-wheel-schema" && req.method === "PUT") {
      const body = await readJsonBody(req);
      const variant = body.variant || "ingatlan";
      const schema = await saveIngatlanWheelSchema(body.schema || body, variant);
      sendJson(res, 200, { ok: true, schema, variant });
      return;
    }

    if (pathname === "/api/level1/search-cylinder" && req.method === "PUT") {
      const body = await readJsonBody(req);
      const menu = await saveSearchCylinderMenu(body.menu || body);
      sendJson(res, 200, {
        ok: true,
        menu,
        items: menu.items,
        imagePresets: SEARCH_CYLINDER_IMAGE_PRESETS,
      });
      return;
    }

    if (pathname === "/api/level1/search-cylinder/admin" && req.method === "GET") {
      const menu = await getSearchCylinderMenu();
      sendJson(res, 200, {
        menu,
        items: menu.items,
        imagePresets: SEARCH_CYLINDER_IMAGE_PRESETS,
      });
      return;
    }

    if (pathname === "/api/level1/kivitel-menu/admin" && req.method === "GET") {
      const menu = await getKivitelMenu();
      sendJson(res, 200, { menu, items: menu.items });
      return;
    }

    if (pathname === "/api/level1/kivitel-menu" && req.method === "PUT") {
      const body = await readJsonBody(req);
      const menu = await saveKivitelMenu(body.menu || body);
      sendJson(res, 200, { ok: true, menu, items: menu.items });
      return;
    }

    if (pathname === "/api/level1/ingatlan-tipus-fields/admin" && req.method === "GET") {
      const config = await getIngatlanTipusFieldsConfig();
      sendJson(res, 200, { config });
      return;
    }

    if (pathname === "/api/level1/ingatlan-tipus-fields" && req.method === "PUT") {
      const body = await readJsonBody(req);
      const config = await saveIngatlanTipusFieldsConfig(body.config || body);
      sendJson(res, 200, { ok: true, config });
      return;
    }

    if (pathname === "/api/level1/partner-profiles" && req.method === "GET") {
      sendJson(res, 200, { partners: await listPartnerProfilesForAdmin() });
      return;
    }

    const partnerProfileMatch = pathname.match(/^\/api\/level1\/partner-profiles\/(\d+)$/);
    if (partnerProfileMatch && req.method === "PATCH") {
      const body = await readJsonBody(req);
      const profile = await reviewPartnerProfile(Number(partnerProfileMatch[1]), {
        status: body.status,
        reviewedBy: admin.username,
      });
      sendJson(res, 200, { ok: true, profile });
      return;
    }

    if (pathname === "/api/level1/akku-search-menu/admin" && req.method === "GET") {
      const { menu, live, source } = await readAkkuSearchMenuRaw();
      sendJson(res, 200, {
        menu,
        items: menu.items,
        section: publicAkkuSearchSection(menu),
        live,
        source: source || null,
      });
      return;
    }

    if (pathname === "/api/level1/akku-search-menu" && req.method === "PUT") {
      const body = await readJsonBody(req);
      const menu = await saveAkkuSearchMenu(body.menu || body);
      const section = publicAkkuSearchSection(menu);
      sendJson(res, 200, {
        ok: true,
        live: true,
        menu,
        items: menu.items,
        section,
        updatedAt: menu.updatedAt || null,
      });
      return;
    }

    if (pathname === "/api/level1/users" && req.method === "GET") {
      sendJson(res, 200, { users: await listWebUsersForAdmin() });
      return;
    }

    if (pathname === "/api/level1/users" && req.method === "POST") {
      // Tesztfiókok: Vercel (vagy explicit env) — ne legyen S1-en alapból.
      const allow =
        process.env.VERCEL === "1" ||
        process.env.ALLOW_ADMIN_TEST_USERS === "1" ||
        process.env.NODE_ENV !== "production";
      if (!allow) {
        sendJson(res, 403, {
          error: "Tesztfiók létrehozás csak Vercelen engedélyezett (ALLOW_ADMIN_TEST_USERS).",
        });
        return;
      }
      const body = await readJsonBody(req);
      const user = await adminCreateWebUser({
        email: body.email ?? body.username,
        displayName: body.displayName ?? body.name,
        password: body.password,
        accountType: body.accountType,
      });

      let listing = null;
      const telepules = String(body.listingTelepules ?? body.telepules ?? "").trim();
      const listingTitle = String(body.listingTitle ?? body.hirdetes_cime ?? "").trim();
      if (telepules || listingTitle) {
        const title =
          listingTitle ||
          [body.listingGyartmany, body.listingModell].filter(Boolean).join(" ").trim() ||
          `${user.displayName || "Teszt"} autó`;
        listing = await saveListing(
          {
            hirdetes_cime: title,
            hirdetes_vertical: String(body.listingVertical || "auto").trim() || "auto",
            gyartmany: String(body.listingGyartmany ?? "").trim(),
            modell: String(body.listingModell ?? "").trim(),
            telepules,
            megye: String(body.listingMegye ?? "").trim(),
            vetelar: String(body.listingPrice ?? "").trim() || "1 000 000 Ft",
            leiras: "Admin teszt hirdetés (térkép / fiók próba).",
          },
          null,
          { status: "feladott", userId: user.id }
        );
      }

      sendJson(res, 200, {
        ok: true,
        user: await getUserDetailsForAdmin(user.id),
        listing: listing
          ? { id: listing.id, title: listing.hirdetes_cime, status: listing.status }
          : null,
      });
      return;
    }

    if (pathname === "/api/level1/visitors" && req.method === "GET") {
      const url = new URL(req.url || "/", "http://local");
      const range = url.searchParams.get("range") || "all";
      const from = url.searchParams.get("from") || "";
      const to = url.searchParams.get("to") || "";
      const stats = await getVisitorAdminStats({ range, from, to });
      sendJson(res, 200, { ...stats, blockedIps: await listBlockedIps() });
      return;
    }

    if (pathname === "/api/level1/visitors/ip-hits" && req.method === "GET") {
      const url = new URL(req.url || "/", "http://local");
      const ip = url.searchParams.get("ip") || "";
      const range = resolveVisitorTimeRange(
        url.searchParams.get("range") || "all",
        url.searchParams.get("from") || "",
        url.searchParams.get("to") || ""
      );
      sendJson(res, 200, {
        ...await getVisitorHitsByIp(ip, { from: range.from, to: range.to }),
        range: {
          key: range.key,
          label: range.label,
          from: range.from ? range.from.toISOString() : null,
          to: range.to ? range.to.toISOString() : null,
        },
      });
      return;
    }

    if (pathname === "/api/level1/visitors/block" && req.method === "POST") {
      const body = await readJsonBody(req);
      sendJson(res, 200, { ok: true, blockedIps: await blockIp(body.ip) });
      return;
    }

    if (pathname === "/api/level1/visitors/unblock" && req.method === "POST") {
      const body = await readJsonBody(req);
      sendJson(res, 200, { ok: true, blockedIps: await unblockIp(body.ip) });
      return;
    }

    if (pathname === "/api/level1/visitors/init" && req.method === "POST") {
      const result = await ensureSupabaseVisitorSchema({ force: true });
      if (!result.ok) {
        sendJson(res, 503, result);
        return;
      }
      sendJson(res, 200, { ok: true, ...result, stats: await getVisitorAdminStats() });
      return;
    }

    const visitorHitsMatch = pathname.match(/^\/api\/level1\/visitors\/([a-f0-9]{32})\/hits$/i);
    if (visitorHitsMatch && req.method === "GET") {
      const url = new URL(req.url || "/", "http://local");
      const range = resolveVisitorTimeRange(
        url.searchParams.get("range") || "all",
        url.searchParams.get("from") || "",
        url.searchParams.get("to") || ""
      );
      sendJson(res, 200, {
        hits: await getVisitorPageHits(visitorHitsMatch[1], { from: range.from, to: range.to }),
      });
      return;
    }

    if (pathname === "/api/level1/banned-users" && req.method === "GET") {
      sendJson(res, 200, { users: await listBannedUsers() });
      return;
    }

    if (pathname === "/api/level1/banned-users/unban" && req.method === "POST") {
      const body = await readJsonBody(req);
      sendJson(res, 200, { ok: true, users: await unbanEmail(body.email) });
      return;
    }

    const banMatch = pathname.match(/^\/api\/level1\/users\/(\d+)\/ban$/);
    if (banMatch && req.method === "POST") {
      const userId = Number(banMatch[1]);
      const details = await getUserDetailsForAdmin(userId).catch(() => null);
      if (!details) {
        sendJson(res, 404, { error: "Nincs ilyen felhasználó." });
        return;
      }
      const fromDetails = (details.listings || [])
        .map((row) => Number(row.id))
        .filter((id) => Number.isFinite(id) && id > 0);
      let deletedFromDetails = 0;
      for (const listingId of fromDetails) {
        try {
          await deleteListing(listingId);
          deletedFromDetails += 1;
        } catch (error) {
          console.warn("Tiltás hirdetés törlés:", listingId, error?.message || error);
        }
      }
      const result = await deleteUserAccount(userId);
      const deletedListings = Math.max(deletedFromDetails, Number(result?.deletedListings) || 0);
      const banned = await banUserRecord({
        email: details.email,
        displayName: details.displayName,
        accountType: details.accountType || details.profile?.accountType,
        companyActivities: details.companyActivities || details.profile?.companyActivities,
        formerUserId: userId,
        listingCountAtBan: Number(details.listingCount) || fromDetails.length,
        deletedListings,
        profileSnapshot: details.profile || {},
      });
      sendJson(res, 200, { ok: true, banned, deletedListings });
      return;
    }

    const userMatch = pathname.match(/^\/api\/level1\/users\/(\d+)$/);
    if (userMatch) {
      const userId = Number(userMatch[1]);
      if (req.method === "GET") {
        sendJson(res, 200, { user: await getUserDetailsForAdmin(userId) });
        return;
      }
      if (req.method === "PATCH") {
        const body = await readJsonBody(req);
        if (body.adminFlags && typeof body.adminFlags === "object") {
          const details = await getUserDetailsForAdmin(userId);
          const nextFlags = mergeAdminFlagsPatch(details?.profile || details?.profileJson || {}, body.adminFlags);
          await mergeUserProfileJson(userId, { adminFlags: nextFlags });
          try {
            const { clearBoostOwnerIdsCache } = await import("./boost-owners.mjs");
            clearBoostOwnerIdsCache();
          } catch {
            /* ignore */
          }
        }
        const user = await updateWebUserForAdmin(userId, {
          email: body.email,
          displayName: body.displayName,
          emailVerified: body.emailVerified,
          profileJson: body.profileJson,
        });
        sendJson(res, 200, { user: await getUserDetailsForAdmin(userId).catch(() => user) });
        return;
      }
      if (req.method === "DELETE") {
        const details = await getUserDetailsForAdmin(userId).catch(() => null);
        const fromDetails = (details?.listings || [])
          .map((row) => Number(row.id))
          .filter((id) => Number.isFinite(id) && id > 0);
        let deletedFromDetails = 0;
        for (const listingId of fromDetails) {
          try {
            await deleteListing(listingId);
            deletedFromDetails += 1;
          } catch (error) {
            console.warn("Felhasználó hirdetés törlés:", listingId, error?.message || error);
          }
        }
        const result = await deleteUserAccount(userId);
        sendJson(res, 200, {
          ok: true,
          deletedListings: Math.max(deletedFromDetails, Number(result?.deletedListings) || 0),
        });
        return;
      }
    }

    if (pathname === "/api/level1/listings" && req.method === "GET") {
      const url = new URL(req.url || "/", "http://localhost");
      const vertical = url.searchParams.get("vertical") || null;
      const listings = await listListingsWithPreview({ limit: 300, vertical });
      const users = await listWebUsersForAdmin();
      const emailById = new Map(
        users.map((user) => [Number(user.id), String(user.email || "").trim()]).filter(([id]) => id > 0)
      );
      const accountTypeById = new Map(
        users
          .map((user) => [
            Number(user.id),
            String(user.accountType || "private").toLowerCase() === "business" ? "business" : "private",
          ])
          .filter(([id]) => id > 0)
      );
      sendJson(
        res,
        200,
        {
          listings: listings.map((row) => {
            const ownerUserId =
              Number(row.user_id || row.owner_user_id || row.form?.owner_user_id || 0) || null;
            return {
              id: row.id,
              title: row.hirdetes_cime,
              status: row.status,
              gyartmany: row.gyartmany,
              tipus: row.tipus,
              vertical: resolveListingVertical(row),
              subtype:
                row.hirdetes_alkategoria ||
                row.preview?.filter?.hirdetes_alkategoria ||
                row.form?.hirdetes_alkategoria ||
                "",
              ownerUserId,
              ownerEmail: (ownerUserId && emailById.get(ownerUserId)) || "",
              ownerAccountType: (ownerUserId && accountTypeById.get(ownerUserId)) || "",
              updatedAt: row.updated_at,
              imageUrl: row.preview?.imageUrl || "",
            };
          }),
        }
      );
      return;
    }

    if (pathname === "/api/level1/backups" && req.method === "GET") {
      sendJson(res, 200, { ...listAdminBackups(), categories: ADMIN_BACKUP_CATEGORIES });
      return;
    }

    if (pathname === "/api/level1/backups" && req.method === "POST") {
      const body = await readJsonBody(req);
      sendJson(
        res,
        200,
        await createAdminBackup({
          createdBy: admin.username,
          note: body?.note || "",
        })
      );
      return;
    }

    const backupPreviewMatch = pathname.match(/^\/api\/level1\/backups\/([^/]+)\/preview$/);
    if (backupPreviewMatch && req.method === "POST") {
      const body = await readJsonBody(req);
      sendJson(
        res,
        200,
        await previewAdminBackup(decodeURIComponent(backupPreviewMatch[1]), {
          category: body?.category || "all",
          userEmail: body?.userEmail ?? null,
          userId: body?.userId ?? null,
          listingId: body?.listingId ?? null,
        })
      );
      return;
    }

    const backupRestoreMatch = pathname.match(/^\/api\/level1\/backups\/([^/]+)\/restore$/);
    if (backupRestoreMatch && req.method === "POST") {
      const body = await readJsonBody(req);
      sendJson(
        res,
        200,
        await restoreAdminBackup(decodeURIComponent(backupRestoreMatch[1]), {
          category: body?.category || "all",
          userEmail: body?.userEmail ?? null,
          userId: body?.userId ?? null,
          listingId: body?.listingId ?? null,
          listingIds: body?.listingIds || null,
          forceStatus: body?.forceStatus || null,
        })
      );
      return;
    }

    const listingMatch = pathname.match(/^\/api\/level1\/listings\/(\d+)$/);
    if (listingMatch && req.method === "PATCH") {
      const body = await readJsonBody(req);
      await updateListingStatus(Number(listingMatch[1]), body.status);
      sendJson(res, 200, { ok: true });
      return;
    }

    if (listingMatch && req.method === "DELETE") {
      await deleteListing(Number(listingMatch[1]));
      sendJson(res, 200, { ok: true });
      return;
    }

    sendJson(res, 404, { error: "Ismeretlen admin API." });
  } catch (error) {
    const code = error.code;
    const message = error.message ?? "Szerver hiba.";
    const status =
      error.status ||
      (code === "LOCKED" ? 423 : code === "INVALID" ? 401 : code === "SMTP_NOT_CONFIGURED" ? 503 : 400);
    sendJson(res, status, { error: message, code: code || null });
  }
}
