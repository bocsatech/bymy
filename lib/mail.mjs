...
function isTransientSmtpError(error) {
  ...
  return /EBUSY|ETIMEDOUT|ECONNRESET|ECONNREFUSED|getaddrinfo|EAI_AGAIN|socket hang up/i.test(msg);
}

async function sendMailOnce(cfg, { to, subject, text, html }) {
...
export async function sendMail({ to, subject, text, html }) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      return await sendMailOnce(cfg, { to, subject, text, html });
    } catch (error) {
      lastError = error;
      if (!isTransientSmtpError(error) || attempt >= 2) throw error;
