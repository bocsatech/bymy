...
import {
  ...
  applyAkkuMenuToFormLayoutCells,
...
const DEFAULT_AKKU_IDS = new Set([
  "akkumulator_kwh",
...
export async function readAkkuSearchMenuRaw() {
  ...
  // Ha még nincs külön KV mentés: a személyautó kereső form-layout az igazság
  try {
    const layout = await getFormLayout("szemelyauto-search");
    const menu = menuFromFormLayoutCells(layout?.cells || [], {
      title: "Akkumulátor és hatótáv adatok",
    });
    const hasAkkuCells = (layout?.cells || []).some((c) =>
      DEFAULT_AKKU_IDS.has(String(c?.field_key || ""))
    );
    return { menu, live: Boolean(hasAkkuCells && layout?.live), source: "form-layout" };
  } catch {
...
export async function saveAkkuSearchMenu(menu) {
  ...
  const verified = normalizeAkkuSearchMenu(parseKvJson(value, { items: [] }));

  // Szinkron a kereső elrendezéssel (Több szűrő + Részletes keresés ugyanazt lássa)
  try {
    const layout = await getFormLayout("szemelyauto-search");
    const cells = Array.isArray(layout?.cells) ? layout.cells : [];
    const changed = applyAkkuMenuToFormLayoutCells(cells, verified);
    if (changed > 0 || cells.some((c) => DEFAULT_AKKU_IDS.has(String(c?.field_key || "")))) {
      layout.cells = cells;
      await saveFormLayout(layout, "szemelyauto-search");
    }
  } catch (error) {
    console.warn("Akku menü → form-layout szinkron:", error?.message || error);
  }
