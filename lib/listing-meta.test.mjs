...
test("mergeProtectedCells replaceImport: régi fotók nem maradnak", () => {
  const merged = mergeProtectedCells(
    [{ field_key: "hasznaltauto_hirdetes_id", label: "HA", value: "1", step: 9 }],
    [
      { field_key: "fotok", label: "Fotók", value: "/img/old.jpg", step: 4 },
      { field_key: "owner_user_id", label: "Tulajdonos", value: "7", step: 9 },
      { field_key: "views_web", label: "Web", value: "9", step: 9 },
      { field_key: "km", label: "Km", value: "1", step: 1 },
    ],
    { preserveKeys: ["owner_user_id", "views_web", "views_app"], dropEmptyFotok: true }
  );
  const byKey = Object.fromEntries(merged.map((cell) => [cell.field_key, cell.value]));
  assert.equal(byKey.hasznaltauto_hirdetes_id, "1");
  assert.equal(byKey.owner_user_id, "7");
  assert.equal(byKey.views_web, "9");
  assert.equal(byKey.fotok, undefined);
  assert.equal(byKey.km, undefined);
});
...
test("canManageListing: csak a tulajdonos, vendég soha", () => {
  assert.equal(canManageListing({ user_id: null }, { id: 2 }), false);
