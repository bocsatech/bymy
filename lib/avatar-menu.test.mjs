...
test("beallitasok Fiókom menü: dealer + fiók tételek", () => {
  ...
  assert.match(nav, /Mentett kereséseim/);
  assert.match(nav, /Üzenetek/);
  assert.match(nav, /Megjelenés/);
  assert.match(nav, /Beállítások/);
