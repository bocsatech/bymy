...
test('loadEnvFiles: .env.local felülírja a .env Supabase/PORT értékeit', () => {
  const dir = mkdtempSync(join(tmpdir(), 'bymy-env-'));
  const backup = { ...process.env };
  ...
  try {
    ...
    writeFileSync(join(dir, '.env.local'), 'SUPABASE_URL=https://bymy.hu\n');

    loadEnvFiles(dir);

    assert.equal(process.env.PORT, '3456');
    assert.equal(process.env.SUPABASE_URL, 'https://bymy.hu');
    assert.equal(process.env.AUX, '1');
  } finally {
    for (const key of Object.keys(process.env)) {
      if (!(key in backup)) delete process.env[key];
    }
