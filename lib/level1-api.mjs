...
import { listWebUsersForAdmin, deleteUserAccount } from "./web-users-store.mjs";
...
export async function handleLevel1Api(req, res, pathname) {
  try {
    ...
    if (pathname === "/api/level1/form-layout" && req.method === "GET") {
      sendJson(res, 200, { layout: await getFormLayout() });
      return;
    }
    ...
    if (pathname === "/api/level1/form-layout" && req.method === "PUT") {
      const body = await readBody(req);
      const layout = await saveFormLayout(body.layout || body);
      sendJson(res, 200, { ok: true, layout });
      return;
    }
    ...
    const userDel = pathname.match(/^\/api\/level1\/users\/(\d+)$/);
    if (userDel && req.method === "DELETE") {
      await deleteUserAccount(Number(userDel[1]));
