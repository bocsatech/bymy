...
test("getPartnerRecommendations: ingatlan vertical csak ingatlan kategóriák", async () => {
  const tempDir = mkdtempSync(join(tmpdir(), "autosweb-partners-"));
  process.env.AUTOSWEB_DB_PATH = join(tempDir, "test.db");
  ...
  savePartner({
    name: "Fejér Értékbecslő",
    address: "Fő u. 9.",
    postal_code: "8000",
    phone: "+36 22 444 5555",
    is_active: true,
    is_paid: true,
    services: ["ertekbecsles"],
  });

  savePartner({
    name: "Autószerelő Fejér",
    address: "Gép u. 1.",
    postal_code: "8000",
    phone: "+36 22 555 6666",
    is_active: true,
    is_paid: true,
    services: ["autoszerelo"],
  });

  const result = getPartnerRecommendations("8000", { vertical: "ingatlan" });
  assert.equal(result.vertical, "ingatlan");
  assert.ok(result.categories.every((c) => c.vertical === "ingatlan"));
  assert.ok(result.categories.some((c) => c.id === "ertekbecsles"));
  assert.ok(!result.categories.some((c) => c.id === "autoszerelo"));
  const ertek = result.categories.find((c) => c.id === "ertekbecsles");
  assert.equal(ertek.partners.length, 1);
  assert.equal(ertek.partners[0].name, "Fejér Értékbecslő");
