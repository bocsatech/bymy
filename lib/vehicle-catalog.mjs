...
function normalizeHeader(header) {
  ...
  if (key === "gyartasiev" || key === "gyartasi_ev") return "evtol";
  if (key === "ajtok" || key === "ajtoszam") return "ajtok";
  if (key === "uzemanyag" || key === "motor") return "uzemanyag";
  return key;
}
  ...
  for (const row of rows) {
    ...
    if (!existing) {
      byName.set(tipus, {
        ...
        evTol,
        evIg,
        ajtok: row.ajtok ? [row.ajtok] : [],
        uzemanyag: row.uzemanyag ? [row.uzemanyag] : [],
      });
      continue;
    }
    ...
    for (const [key, value] of [["ajtok", row.ajtok], ["uzemanyag", row.uzemanyag]]) {
      if (value && !existing[key].includes(value)) existing[key].push(value);
    }
  }
...
/** Régi mentés: tipusok = ["A4 2.0 TDI"]. Új: részletes típusadatok. */
...
/** Márkák + modellek + típusfüggő évjárat/ajtó/üzemanyag adatok. */
...
export function importVehicleCatalogFromCsv(csvPath, outPath = CATALOG_PATH) {
  ...
  const rows = paths.flatMap((resolved) => {
    if (!existsSync(resolved)) throw new Error(`CSV nem található: ${resolved}`);
    return parseCsvText(readFileSync(resolved, "utf8"));
  });
  if (!rows.length) {
    throw new Error("Üres vagy hibás CSV.");
  }
  const catalog = slimVehicleCatalog(buildVehicleCatalog(rows, paths.join(", ")));
