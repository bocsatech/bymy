...
import { isEmailBannedSync } from "./banned-users.mjs";
...
export function findOrCreateOAuthUser(identity) {
  ...
  if (byIdentity) {
    if (Number(byIdentity.email_verified) !== 1) {
  ...
  // Orphan identity (user deleted, FK not cleaned) — drop before re-register.
  db.prepare(
    `DELETE FROM web_user_identities WHERE provider = ? AND provider_subject = ?`
  ).run(provider, subject);
  ...
  if (!userRow) {
    // Belépés: ne hozzon létre fiókot (törlés után se). Új fiók csak regisztrációnál.
...
export function registerUser(email, password, passwordConfirm, accountType) {
  ...
  if (isEmailBannedSync(normalized)) {
...
export function adminCreateWebUser({
  email,
  displayName,
  password,
  accountType = "private",
  ...
  const info = db
    .prepare(
      `INSERT INTO web_users (
        email, password_salt, password_hash, display_name, profile_json,
        email_verified, activation_token_hash, activation_expires_at
      ) VALUES (?, ?, ?, ?, ?, 1, NULL, NULL)`
    )
    .run(normalized, salt, hash, name, profileJson);

  return getUserDetailsForAdmin(Number(info.lastInsertRowid));
}
...
export function deleteUserAccount(userId) {
  ...
  const row = db.prepare(`SELECT email FROM web_users WHERE id = ?`).get(uid);
  ...
  try {
    const owned = db
      .prepare(
        `SELECT DISTINCT listing_id AS id FROM listing_cells
         WHERE field_key = 'owner_user_id'
           AND TRIM(value) != ''
           AND CAST(TRIM(value) AS INTEGER) = ?`
      )
      .all(uid);
    const wipeCells = db.prepare(`DELETE FROM listing_cells WHERE listing_id = ?`);
    const wipeListing = db.prepare(`DELETE FROM listings WHERE id = ?`);
    for (const { id } of owned) {
      const listingId = Number(id);
      if (!Number.isFinite(listingId) || listingId <= 0) continue;
      wipeCells.run(listingId);
      wipeListing.run(listingId);
      deletedListings += 1;
    }
  } catch {
    /* listings tables may be missing in isolated auth tests */
  }

  db.prepare(`DELETE FROM web_sessions WHERE user_id = ?`).run(uid);
  try {
    db.prepare(`DELETE FROM web_user_identities WHERE user_id = ?`).run(uid);
  } catch {
    /* older schemas */
  }
  db.prepare(`DELETE FROM web_users WHERE id = ?`).run(uid);
