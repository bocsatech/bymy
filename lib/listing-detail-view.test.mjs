...
test("hirdetésnézet a feltöltött autóadatokból, magyar címkékkel", () => {
  ...
  assert.equal(view.power, "112 kW, 152 LE");
  assert.ok(view.motorSpecs.some((row) => row.label === "Sebességváltó" && row.value.includes("automata")));
  assert.ok(view.motorSpecs.some((row) => row.label === "Üzemanyag" && /Benzin/i.test(row.value)));
  assert.ok(view.vehicleSpecs.some((row) => row.label === "Jármű típusa" && /crossover|terepjáró/i.test(row.value)));
  assert.ok(view.vehicleSpecs.some((row) => row.label === "Kárpit színe" && row.value === "Szürke"));
  assert.ok(view.vehicleSpecs.some((row) => row.label === "Saját tömeg" && /1845/.test(row.value)));
  assert.ok(view.vehicleSpecs.some((row) => row.label === "Teljes tömeg" && /2410/.test(row.value)));
  assert.ok(view.vehicleSpecs.some((row) => row.label === "Klíma fajtája" && /digitális klíma/i.test(row.value)));
  assert.ok(view.vehicleSpecs.some((row) => row.label === "Km. óra állás" && /142\s000/.test(row.value)));
  assert.ok(view.equipment.includes("tempomat"));
  assert.ok(view.equipmentGroups.some((g) => g.title === "Műszaki" && g.items.includes("tempomat")));
  assert.equal(view.equipment.includes("digitális klíma"), false);
  assert.ok(view.basics.some((row) => row.label === "Sebességváltó"));
  assert.ok(view.bodyTech.some((row) => row.label === "Jármű típusa"));
