import { pickValue, cleanText, normalizeKey, splitChainedHaValue } from "./parse-listing.mjs";
...
export const FIELD_ALIASES = {
  ...
  finanszirozas: ["finanszírozás", "finanszirozas", "hitel"],
  teljesitmeny: ["teljesítmény", "teljesitmeny", "lóerő", "kw"],
...
export function mapToltoCsatlakozas(value) {
  ...
  if (!v) return "";
  if (v.includes("type 1") || v.includes("type1") || v.includes("j1772")) return "Type 1";
  if (v.includes("type 2") || v.includes("type2") || v.includes("mennekes")) return "Type 2";
...
export function transformFieldValue(field, value) {
  let raw = splitChainedHaValue(value);
  if (!raw) return "";
  ...
  switch (field) {
    ...
    case "gyartasi_ev":
    case "forgalomba_helyezes_ev":
    case "muszaki_ev": {
      const ym = parseYearMonth(raw);
      return ym.ev || "";
    }
    case "gyartasi_honap":
    case "forgalomba_helyezes_honap":
    case "muszaki_honap": {
      const ym = parseYearMonth(raw);
      if (ym.honap) return ym.honap;
      const n = parseInt(digits(raw), 10);
      return Number.isFinite(n) && n >= 1 && n <= 12 ? String(n) : "";
    }
    case "szin":
    case "karpit1":
    case "karpit2": {
      raw = splitChainedHaValue(raw);
      if (/\b(?:saját tömeg|össztömeg|hengerűrtartalom)\b/i.test(raw) || /csomagtartó|csomagtarto/i.test(raw)) {
        raw = splitChainedHaValue(raw.split(/\b(?:saját tömeg|össztömeg)\b/i)[0]);
      }
      const looksColor =
        /(fehér|feher|fekete|szürke|szurke|ezüst|ezust|kék|kek|piros|zöld|zold|sárga|sarga|barna|bézs|bezs|beige|narancs|lila|bordó|bordo|arany|bron|metál|metal|gyöngy|gyongy|grey|gray|black|white|silver|blue|red|green|yellow|brown|orange|purple|pearl|navy|világos|vilagos|sötét|sotet)/i.test(
          raw
        );
      if (field === "szin" && !looksColor) return "";
      if (
        field === "szin" &&
        /(tető|teto|kamera|sensor|ülés|ules|navi|led|radar|abs\b|multimédia|multimedia|navigáció|tolótető)/i.test(raw) &&
        !looksColor
      ) {
        return "";
      }
      // „Kárpit színe (1): …” benne a szín mezőben
      if (field === "szin") {
        raw = raw.split(/\s+Kárpit színe/i)[0].trim();
      }
...
export function applyFieldMap(data, m, parsed = {}) {
  for (const [label, value] of Object.entries(m)) {
    ...
    if (!transformed) {
      // Rossz/összefolyt érték: töröld a mezőt, ha eddig is szemét volt
      if (
        (field === "szin" || field === "karpit1" || field === "karpit2") &&
        data[field] &&
        !transformFieldValue(field, data[field])
      ) {
        data[field] = "";
      }
      continue;
    }
    if (field === "gyartasi_ev" || field === "forgalomba_helyezes_ev" || field === "muszaki_ev") {
      const ym = parseYearMonth(value);
      if (ym.ev && !data[field]) data[field] = ym.ev;
      const monthKey = field.replace(/_ev$/, "_honap");
      if (ym.honap && !data[monthKey]) data[monthKey] = ym.honap;
      continue;
    }
    if (!data[field]) {
    ...
    // Ha a meglévő érték összefolyt / rosszabb, cseréld
    if (
      (field === "szin" || field === "karpit1" || field === "karpit2") &&
      transformFieldValue(field, data[field]) !== data[field]
    ) {
      data[field] = transformed;
    }
  }
