...
function joinBrandModelTipus(gyartmany, modell, tipus) {
  const base = [gyartmany, modell].filter(Boolean).join(" ");
  if (isPlaceholderTipus(tipus) || isStubVehicleName(tipus)) return base;
  const tipusClean = sanitizeListingFieldValue(tipus);
  if (!tipusClean) return base;
  const b = base.toLowerCase();
  const t = tipusClean.toLowerCase();
  const m = String(modell || "").toLowerCase();
  if (!b) return tipusClean;
  if (t === b || t === m) return base;
  if (t.length >= 2 && b.includes(t)) return base;
  if (t.startsWith(`${b} `)) return tipusClean;
  if (m && t.startsWith(`${m} `)) {
    return [gyartmany, tipusClean].filter(Boolean).join(" ");
  }
  return `${base} ${tipusClean}`;
}
...
export function composeVehicleTitle(data = {}) {
  ...
  const parts = joinBrandModelTipus(gyartmany, modell, data.tipus);
  if (!parts) return "";
