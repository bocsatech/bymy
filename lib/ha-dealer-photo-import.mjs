...
export async function saveDealerPhotoImportPages({ pages = [], userId = null, limit = MAX_IMPORT_BATCH } = {}) {
  ...
  for (const page of list) {
    ...
    try {
      ...
      let remoteImage = upgradeHaImageUrl(clean(page?.visibleImage || page?.imageUrl || ""));
      if (remoteImage && isHaThumbImageUrl(remoteImage)) remoteImage = "";
      if (!photo && !/^https?:\/\//i.test(remoteImage)) {
        errors.push({ url, message: "Nem sikerült az első képet menteni (túl kicsi / hiányzó kép)." });
        continue;
      }
      ...
      // Meglévő: teljes cellacsere — csak az importált mezők maradnak (+ tulajdonos/views).
      if (existing?.id) {
        try {
          await clearListingPhotos(existing.id);
        } catch {
          /* fo_kep / fotok törlés best-effort */
        }
      }
