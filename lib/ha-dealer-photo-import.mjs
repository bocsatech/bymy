...
import { buildFormFromPage, MAX_IMPORT_BATCH } from "./ha-import-save.mjs";
...
function buildPhotoOnlyIdentity(page = {}) {
  ...
  const gyartmany = normalizeGyartmanyForCatalog(clean(page.gyartmany || parsed.gyartmany));
  const modell = clean(page.modell || parsed.modell);
...
function patchDealerFormFromMap(form, map = {}) {
  ...
  if (!next.allapot) next.allapot = "Normál";
  return next;
}
...
function buildPhotoOnlyForm(page = {}) {
  ...
  if (html.length <= 400) {
    if (!Object.keys(clientMap).length) return base;
