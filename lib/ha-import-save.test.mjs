...
test("saveExtractedPages: importáló címe és telefonja kerül a hirdetésbe", async () => {
  const dir = mkdtempSync(join(tmpdir(), "bymy-ha-contact-"));
  process.env.DB_BACKEND = "sqlite";
  process.env.AUTOSWEB_DB_PATH = join(dir, "test.db");
  process.env.AUTOSWEB_UPLOADS_PATH = join(dir, "listings");

  const { saveExtractedPages } = await import(`./ha-import-save.mjs?t=${Date.now()}-contact`);
  const { getListing } = await import(`./db.mjs?t=${Date.now()}-contact`);
  const { mergeUserProfileJson } = await import(`./web-users.mjs?t=${Date.now()}-contact`);
  const userId = await makeImportUser("business");
  await mergeUserProfileJson(userId, {
    company: "Teszt Autó Kft.",
    companyStreet: "Import utca 5.",
    companyPostalCode: "2040",
    companyCity: "Budaörs",
    companyPhone: "+36201234567",
  });

  const page = {
    url: "https://www.hasznaltauto.hu/szemelyauto/opel/astra/teszt-77776666",
    listingId: "77776666",
    visibleTitle: "Opel Astra",
    price: "2500000",
    km: "120000",
    year: "2015",
    fuel: "Dízel",
    ...
    map: {
      ...
      Település: "Debrecen",
      "Irányítószám": "4000",
      "Megtekintési cím": "HA utca 99.",
    },
  };
  const result = await saveExtractedPages({ pages: [page], userId });
  assert.equal(result.savedCount, 1);
  const listing = getListing(result.items[0].savedId);
  assert.equal(listing.form.megtekintesi_cim, "Import utca 5.");
  assert.equal(listing.form.iranyitoszam, "2040");
  assert.equal(listing.form.telepules, "Budaörs");
  assert.equal(listing.form.telefon1_korzet, "20");
  assert.equal(listing.form.telefon1_szam, "123 4567");
...
test("összefolyt Szín/Kárpit mezők levágása + Leírás prefix", () => {
  const form = buildFormFromPage({
    url: "https://www.hasznaltauto.hu/szemelyauto/import-23449408",
    listingId: "23449408",
    visibleTitle: "LEXUS NX 300h Comfort",
    brand: "LEXUS",
    model: "NX",
    year: "2018",
    price: "9250000",
    km: "94000",
    visibleDescription: "Leírás MEGKÍMÉLT, SÉRÜLÉSMENTES állapot.",
    map: {
      Márka: "LEXUS",
      Modell: "NX",
      Szín: "Szürke (metál) Saját tömeg: 1 835 kg Össztömeg: 2 420 kg",
      "Kárpit színe (1)": "Szürke Saját tömeg: 1 740 kg Össztömeg:",
      Vételár: "9 250 000 Ft",
      Futásteljesítmény: "94 000 km",
      Évjárat: "2018/8",
    },
  });
  assert.equal(form.szin, "Szürke (metál)");
  assert.equal(form.karpit1, "Szürke");
  assert.doesNotMatch(form.leiras || "", /^leírás/i);
  assert.match(form.leiras || "", /MEGKÍMÉLT/i);
});

test("Mercedes típus bekerül a címbe modell nélkül is", () => {
  const form = buildFormFromPage({
    url: "https://www.hasznaltauto.hu/szemelyauto/import-23346578",
    listingId: "23346578",
    ...
    map: {
      Márka: "MERCEDES-BENZ",
      Típus: "220 T CDI Elegance (Automata) Navi/Tempomat",
      Évjárat: "2004/4",
      Vételár: "1 279 990 Ft",
    },
  });
  assert.match(form.hirdetes_cime, /220 T CDI/i);
  assert.doesNotMatch(form.hirdetes_cime, /^Eladó MERCEDES-BENZ \(2004\)$/i);
});

test("Mercedes import megőrzi kárpit/tömeg/klíma mezőket", () => {
  const form = buildFormFromPage({
    url: "https://admin.hasznaltauto.hu/gyorsnezet/szemelyauto/23113337",
    listingId: "23113337",
    visibleTitle: "MERCEDES-BENZ E 250 CDI 4Matic Classic",
    brand: "MERCEDES-BENZ",
    model: "E",
    year: "2013",
    price: "3999990",
    ...
    map: {
      ...
      "Saját tömeg": "1 845 kg",
      Össztömeg: "2 410 kg",
      "Klíma fajtája": "Digitális kétzónás klíma",
      Sebességváltó: "Automata (7 fokozatú tiptronic) sebességváltó",
      Vételár: "3 999 990 Ft",
      Futásteljesítmény: "358 200 km",
      Évjárat: "2013/12",
    },
    felszereltseg: ["dönthető utasülések", "fűthető első ülés", "tempomat"],
  });
  assert.equal(form.karpit1, "Szürke");
  assert.equal(form.sajat_tomeg, "1845");
  assert.equal(form.ossztomeg, "2410");
  assert.equal(form.klima, "digitális kétzónás klíma");
  assert.equal(form.modell, "E");
  assert.match(form.tipus || "", /^250 CDI/i);
  assert.match(form.hirdetes_cime, /E 250 CDI/i);
  assert.match(form.sebessegvalto || "", /tiptronic/i);
});

test("tetőcsomagtartó nem lesz szín", () => {
  const form = buildFormFromPage({
    url: "https://www.hasznaltauto.hu/szemelyauto/import-11112222",
    listingId: "11112222",
    visibleTitle: "Ford Kuga",
    brand: "FORD",
    model: "KUGA",
    ...
    map: {
      Márka: "FORD",
      Modell: "KUGA",
      Szín: "tetőcsomagtartó",
      Vételár: "7 299 000 Ft",
    },
  });
  assert.equal(form.szin || "", "");
});
