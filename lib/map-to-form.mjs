...
import { applyFieldMap, transformFieldValue } from "./field-key-map.mjs";
import { summarizeImportByStep, formatImportSummary } from "./map-import-summary.mjs";
import { sanitizeListingFieldValue, sanitizeListingPlainText, isListingTitleNoise } from "./listing-preview.mjs";
import {
  findCatalogBrand,
  findCatalogModel,
  normalizeBrand,
...
export function parseTitleParts(title) {
  ...
  // UI / évjárat cím ne legyen márka+modell
  if (!sanitizeListingFieldValue(line1)) {
    return { gyartmany: "", modell: "", rest: "" };
  }
...
function mapKivitel(value) {
  return normalizeKivitel(value);
}

function mapAllapot(value) {
  const v = normalizeKey(value);
  if (!v) return "";
  if (v.includes("optikai")) return "Optikai hibás";
  if (v.includes("kitu") || v.includes("kituno")) return "Kitűnő";
  if (v.includes("megkimelt") || v.includes("megkímélt")) return "Megkímélt";
  if (v.includes("ujszer") || v.includes("újszer")) return "Újszerű";
  if (v.includes("serulesmentes") || v.includes("sérülésmentes")) return "Sérülésmentes";
  if (v.includes("enyhen serult") || v.includes("enyhén sérült")) return "Enyhén sérült";
  if (v.includes("eleje serult") || v.includes("eleje sérült")) return "Eleje sérült";
  if (v.includes("hatulja serult") || v.includes("hátulja sérült")) return "Hátulja sérült";
  if (v.includes("baloldala")) return "Baloldala sérült";
  if (v.includes("jobboldala")) return "Jobboldala sérült";
  if (v.includes("hianyos") || v.includes("hiányos")) return "Hiányos";
  if (v.includes("motorhib")) return "Motorhibás";
  if (v.includes("valtohib") || v.includes("váltóhib")) return "Váltóhibás";
  if (v.includes("elektronika")) return "Elektronika hibás";
  if (v.includes("fekhib") || v.includes("fékhib")) return "Fékhibás";
  if (v.includes("futomu") || v.includes("futómű")) return "Futómű hibás";
  if (v.includes("fodarab") || v.includes("fődarab")) return "Fődarab hibás";
...
function mapOkmanyJelleg(value) {
  const v = normalizeKey(value);
  if (!v) return "";
  if (v.includes("nelkul") || v.includes("nincs okmany")) return "Okmányok nélkül";
  if (v.includes("kulfold") || v.includes("külföld")) return "Külföldi okmányokkal";
  if (v.includes("magyar") || v.includes("forgalmi")) return "Magyar okmányokkal";
  return cleanText(value);
}
...
function inferTipus(titleParts, parsed, m) {
  if (titleParts.rest && !isListingTitleNoise(titleParts.rest)) return titleParts.rest;
  const fromTable = pickValue(m, ["típus", "tipus"]);
  if (fromTable && !isListingTitleNoise(fromTable)) return fromTable;
  const cim = cleanText(parsed.cim || "");
  if (isListingTitleNoise(cim)) return "—";
  const withoutBrandModel = cim
    .replace(new RegExp(`^${titleParts.gyartmany}\\s+`, "i"), "")
    .replace(new RegExp(`^${titleParts.modell}\\s+`, "i"), "")
    .replace(/\s*\((19|20)\d{2}.*\)\s*$/, "")
    .trim();
  if (withoutBrandModel && !isListingTitleNoise(withoutBrandModel)) return withoutBrandModel;
  return cim && !isListingTitleNoise(cim) ? cim : "—";
}
...
function inferKivitelFromText(...texts) {
  ...
  if (v.includes("sedan") || v.includes("szedan") || v.includes("lepcosh")) return "Sedan";
  if (v.includes("egyteru") || v.includes("mpv")) return "Egyterű";
  if (v.includes("kupe") || v.includes("coupe")) return "Coupe";
  if (v.includes("cabrio")) return "Cabrio";
  if (v.includes("pickup") || v.includes("pick-up")) return "Pickup";
  if (v.includes("terepjaro")) return "Terepjáró";
  if (v.includes("buggy")) return "Buggy";
  if (v.includes("kisbusz") || v.includes("minibus")) return "Kisbusz";
  if (v.includes("moped")) return "Mopedautó";
  if (v.includes("sport")) return "Sport";
  if (v.includes("hot rod") || v.includes("hotrod")) return "Hot rod";
  return normalizeKivitel(texts.filter(Boolean).join(" ")) || "";
}
...
export function mapListingToForm(parsed) {
  ...
  const data = {
    ...
    szin: transformFieldValue("szin", pickValue(m, ["szín", "szin"]) || ""),
  ...
  if (data.tipus && isListingTitleNoise(data.tipus)) data.tipus = "";
  if (!data.modell) {
    data.modell =
