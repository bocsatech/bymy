...
export async function deleteUserAccount(userId) {
  ...
  const listingIds = [
    ...new Set((cells || []).map((c) => Number(c.listing_id)).filter((id) => Number.isFinite(id) && id > 0)),
  ];
  if (listingIds.length) {
    // listing_cells + listing_feed cascade via listings(id) ON DELETE CASCADE
    const { error: listErr } = await sb().from("listings").delete().in("id", listingIds);
    if (listErr) throw listErr;
  }

  await sb().from("web_sessions").delete().eq("user_id", uid);
  await sb().from("web_user_identities").delete().eq("user_id", uid);
  await sb().from("web_users").delete().eq("id", uid);
}
