...
import { findListingBySource, saveListing, listMyListings } from "./db-store.mjs";
...
export async function importFromClient(options = {}) {
  ...
  if (autoSave && !(Number.isFinite(userId) && userId > 0)) {
    throw new Error("Csak regisztrált felhasználók importálhatnak.");
  }
  let ownerUser = null;
  let mine = [];
  if (Number.isFinite(userId) && userId > 0) {
    try {
      ...
      ownerUser = { id: userId, profile: owner?.profile || { accountType: "private" } };
    } catch {
      ownerUser = { id: userId, profile: { accountType: "private" } };
    }
    try {
      mine = await listMyListings({ userId, limit: 500 });
    } catch {
      mine = [];
    }
  }
    ...
    try {
      ...
      if (autoSave) {
        ...
        if (!existing) {
          ...
          if (!check.ok) {
            errors.push({ url, message: check.error });
            continue;
          }
        }
        const saved = await saveListing(form, existing?.id ?? null, {
          status: "feladott",
          userId,
        });
        item.savedId = saved?.id ?? null;
        savedCount += 1;
        if (!existing && saved) mine = [...mine, saved];
      }
      items.push(item);
    } catch (error) {
