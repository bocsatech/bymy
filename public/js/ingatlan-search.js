...
import {
  ...
  syncHostClearButton,
  initMenuWheel,
} from "./ingatlan-wheels.js?v=mobFix7";
import {
  closeAllInlineDrums,
  initDrumWheel,
...
import { bindAutoDrumSheet, closeAutoDrumSheet } from "./auto-drum-sheet.js?v=mobFix7";
import {
...
function fillPostSingleWheels(form) {
  ...
  for (const { key, options, emptyLabel } of singles) {
    ...
    initImmoSearchWheel(wheel, { emptyLabel, multiple: false, customInput: false });
    const next = form.querySelector(`[data-wheel="${key}"]`);
    if (next) {
      next.dataset.multiple = "0";
      next.closest(".immo-wheel-wrap")?.classList.remove("immo-wheel-wrap--multi");
      setWheelValue(next, prev);
      syncImmoSearchWheelDisplay(next);
    }
  }
}
...
function syncTipusFieldVisibility(form) {
  ...
  const showAlap =
    areaKeys.has("alapterulet") ||
    areaKeys.has("alapterulet_tol") ||
    areaKeys.has("alapterulet_ig");
  const showTelek =
    areaKeys.has("telekterulet") ||
    areaKeys.has("telekterulet_tol") ||
    areaKeys.has("telekterulet_ig");
  ...
  form.querySelectorAll("[data-schema-field]").forEach((cell) => {
    ...
    if (dual) {
      ...
      let show = group
        ? visible.has(group.tolKey) || visible.has(group.igKey) || visible.has(group.id)
        : visible.has(key);
...
function readForm(form) {
  ...
  out.emelet = readWheel(form.querySelector('[data-wheel="emelet"]'));
...
function setupIngatlanSearchWheels(root, { tipusOpts, tipus2Enabled, defaultUzletag, surface = "search" }) {
  ...
  const postSingleKeys = new Set(["ingatlan_lakas_tipus", "ingatlan_tipus_2"]);
...
async function reloadIngatlanSchemaLayout(root, opts) {
  ...
  setupIngatlanSearchWheels(root, { tipusOpts, tipus2Enabled, defaultUzletag, surface });
...
export async function initIngatlanSearch({
  ...
  resetAdFormIngatlanPickers(root);
  setupIngatlanSearchWheels(root, { tipusOpts, tipus2Enabled, defaultUzletag: initialUz, surface });
