...
import { defaultSearchFormLayout, ensureSearchAkkuBoard } from "./search-form-layout.mjs";
import {
...
import {
  AKKU_SEARCH_MENU_KV_KEY,
  normalizeAkkuSearchMenu,
  publicAkkuSearchSection,
} from "./akku-search-menu.mjs";
import {
  INGATLAN_TIPUS_FIELDS_KV_KEY,
  normalizeIngatlanTipusFieldsConfig,
  publicIngatlanTipusFieldsConfig,
  defaultIngatlanTipusFieldsConfig,
} from "./ingatlan-tipus-fields.mjs";
...
let level1InitPromise = null;

export async function initLevel1() {
  if (level1InitPromise) return level1InitPromise;
  level1InitPromise = initLevel1Once().catch((error) => {
    level1InitPromise = null;
    throw error;
  });
  return level1InitPromise;
}

async function initLevel1Once() {
...
export async function startLevel1Login(username, password) {
  ...
  if (!admin) {
    ...
    const err = new Error("Hibás felhasználónév vagy jelszó.");
    err.code = "INVALID";
    throw err;
  }
...
function parseStoredLayout(value, category) {
  ...
  if (isSearchLayoutCategory(cat)) ensureSearchAkkuBoard(layout);
  return layout;
}

function parseKvJson(value, fallback = {}) {
...
export async function getFormLayout(categoryInput = "szemelyauto") {
  ...
  const sourceCategory = deskPostingLayoutSourceCategory(category);
  const key = layoutKvKey(sourceCategory);
  let value = await readKv(key);
  if (value == null && sourceCategory === "szemelyauto") {
    value = await readKv(LEGACY_LAYOUT_KEY);
  }
  if (value != null && value !== "") {
    const layout = parseStoredLayout(value, sourceCategory);
    if (category !== sourceCategory) layout.category = category;
    return layout;
  }
  ...
  const layout = parseStoredLayout(null, sourceCategory);
  if (category !== sourceCategory) layout.category = category;
  return layout;
}
...
export async function getSearchCylinderMenu() {
  ...
  return normalizeSearchCylinderMenu(parseKvJson(value, { items: DEFAULT_SEARCH_CYLINDER_ITEMS }));
}
...
export async function getKivitelMenu() {
  await initLevel1();
  const value = await readKv(KIVITEL_MENU_KV_KEY);
  if (value == null || value === "") {
    return normalizeKivitelMenu({ items: [] });
  }
  return normalizeKivitelMenu(parseKvJson(value, { items: [] }));
}

export async function saveKivitelMenu(menu) {
  await initLevel1();
  const stored = normalizeKivitelMenu(menu && typeof menu === "object" ? menu : {});
  await writeKv(KIVITEL_MENU_KV_KEY, JSON.stringify(stored));
  return stored;
}

export async function getKivitelMenuPublic() {
  const menu = await getKivitelMenu();
  return {
    labels: publicKivitelLabels(menu),
    items: menu.items.filter((item) => item.enabled !== false),
  };
}

export async function getAkkuSearchMenu() {
  return (await readAkkuSearchMenuRaw()).menu;
}

export async function readAkkuSearchMenuRaw() {
  await initLevel1();
  const value = await readKv(AKKU_SEARCH_MENU_KV_KEY);
  const liveKv = value != null && value !== "";
  if (liveKv) {
    return { menu: normalizeAkkuSearchMenu(parseKvJson(value, { items: [] })), live: true, source: "kv" };
  }
  return { menu: normalizeAkkuSearchMenu({ items: [] }), live: false, source: "default" };
}

export async function saveAkkuSearchMenu(menu) {
  await initLevel1();
  const stored = normalizeAkkuSearchMenu(menu && typeof menu === "object" ? menu : {});
  stored.updatedAt = new Date().toISOString();
  const json = JSON.stringify(stored);
  await writeKv(AKKU_SEARCH_MENU_KV_KEY, json);
  const value = await readKv(AKKU_SEARCH_MENU_KV_KEY);
  if (value == null || value === "") {
    const err = new Error("A mentés nem ellenőrizhető — a tároló üres maradt.");
    err.code = "KV_VERIFY_FAILED";
    throw err;
  }
  return normalizeAkkuSearchMenu(parseKvJson(value, { items: [] }));
}

export async function getAkkuSearchMenuPublic() {
...
export async function getIngatlanTipusFieldsConfig() {
  await initLevel1();
  const value = await readKv(INGATLAN_TIPUS_FIELDS_KV_KEY);
  if (value == null || value === "") {
    return normalizeIngatlanTipusFieldsConfig(defaultIngatlanTipusFieldsConfig());
  }
  return normalizeIngatlanTipusFieldsConfig(parseKvJson(value, {}));
}

export async function saveIngatlanTipusFieldsConfig(config) {
  await initLevel1();
  const stored = normalizeIngatlanTipusFieldsConfig(config && typeof config === "object" ? config : {});
  const payload = { version: stored.version, by_tipus: stored.by_tipus };
  await writeKv(INGATLAN_TIPUS_FIELDS_KV_KEY, JSON.stringify(payload));
  return normalizeIngatlanTipusFieldsConfig(payload);
}

export async function getIngatlanTipusFieldsPublic() {
  return publicIngatlanTipusFieldsConfig(await getIngatlanTipusFieldsConfig());
}
