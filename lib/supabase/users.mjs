...
export async function getUserDetailsForAdmin(userId) {
  ...
  let profileJson = {};
  try {
    profileJson = row.profile_json ? (typeof row.profile_json === "string" ? JSON.parse(row.profile_json) : row.profile_json) : {};
  } catch {
...
export async function updateWebUserForAdmin(userId, patch = {}) {
  ...
  if (patch.displayName !== undefined) {
    update.display_name = String(patch.displayName ?? "").trim();
  }
  if (patch.emailVerified !== undefined) {
    update.email_verified = patch.emailVerified ? 1 : 0;
  }
  if (patch.profileJson !== undefined) {
    ...
    update.profile_json = JSON.stringify(profileJson ?? {});
  }
  if (Object.keys(update).length === 0) return getUserDetailsForAdmin(userId);
