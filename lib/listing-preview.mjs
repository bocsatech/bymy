...
export function composeVehicleTitle(data = {}) {
  ...
  const gyartmany = gyartmanyRaw ? normalizeBrand(gyartmanyRaw) : "";
  let modell = isStubVehicleName(data.modell) ? "" : sanitizeListingFieldValue(data.modell);
  if (!modell) {
    modell = findCatalogModel(gyartmany, data.tipus);
  }
  if (!modell) {
    modell = findCatalogModel(gyartmany, data.hirdetes_cime);
  }
